...
//    ( - ¦¤ )^¦Á u = f  in ¦¸,      u = 0  on ?¦¸,      0 < ¦Á,
//
//  To solve this FPDE, we apply the operator ( - ¦¤ )^(-N), where the integer
//  N is given by floor(¦Á). By doing so, we obtain
//
//    ( - ¦¤ )^(¦Á-N) u = ( - ¦¤ )^(-N) f  in ¦¸,      u = 0  on ?¦¸,      0 < ¦Á.
//
//  We first compute the right hand side by solving the integer order PDE
//
//   ( - ¦¤ )^N g = f  in ¦¸, g = ( - ¦¤ )^k g = 0 on ?¦¸, k = 1,..,N-1
//
...
//  A^{-¦Á + N}, where A = - ¦¤ (with associated homogeneous boundary conditions)
//  and (a-N) in (0,1). We approximate the operator
//
//    A^{-¦Á+N} ¡Ö ¦²_{i=0}^M c_i (A + d_i I)^{-1},      d_0 = 0,   d_i > 0,
//
...
//
//    A u_i + d_i u_i = c_i g  in ¦¸,      u_i = 0  on ?¦¸,      i=0,...,M,
//
...
// References:
//
// [1] Nakatsukasa, Y., S¨¨te, O., & Trefethen, L. N. (2018). The AAA algorithm
...
int main(int argc, char *argv[])
{
   ...
   if (verification)
   {
      ...
      // (-¦¤)^¦Á u = (2\pi ^2)^¦Á sin(\pi x) sin(\pi y) on [0,1]^2
   ...
   // ------------------------------------------------------------------------
   // 10. Solve the PDE (-¦¤)^N g = f, i.e. compute g = (-¦¤)^{-1}^N f.
   // ------------------------------------------------------------------------
   ...
   if (power_of_laplace > 0)
   {
      ...
      for (int i = 0; i < power_of_laplace; i++)
      {
         ...
         // 10.5 Visualize the solution g of -¦¤ ^ N g = f in the last step
         if (i == power_of_laplace - 1)
         {
            ...
            if (visualization)
            {
               ...
               oss_f << "Step " << progress_steps++ << ": Solution of PDE -¦¤ ^ "
                     << power_of_laplace
                     << " g = f";
               fout << "solution\n" << mesh << g
      ...
      for (int i = 0; i < coeffs.Size(); i++)
      {
         ...
         if (visualization)
         {
            ...
            oss_u << "Step " << progress_steps + 1
                  << ": Solution of fractional PDE (-¦¤)^" << alpha
                  << " u = f";
            uout << "solution\n" << mesh << u
   ...
   if (verification)
   {
      ...
      {
         case 1:
            manufactured_solution = "sin(¦Ð x)";
            expected_mesh = "inline_segment.mesh";
            break;
         case 2:
            manufactured_solution = "sin(¦Ð x) sin(¦Ð y)";
            expected_mesh = "inline_quad.mesh";
            break;
         default:
            manufactured_solution = "sin(¦Ð x) sin(¦Ð y) sin(¦Ð z)";
