...
//
//                   maximize ÷_次 ?? d?? subject to |???| ≒ 1 in 次, ?? = 0 on ?次,    (?)
//
...
//              ??? := {?? ﹋ R? | |??| ≒ 1}. Our choice is the Hellinger entropy,
//
//                    R(??) = ?( 1 ? |??|2 )^{1/2},
//
...
//
//                 maximize  ÷_次 ?? d?? - 汐??1 D(???,??????)  subject to  ?? = 0 on 次.
//
//              This results in a sequence of functions ( ??? , ??? ),
//
//                      ??? ↙ ??,    ???/|???| ↙ ???    as k ↙ ﹢,
//
//              defined by the nonlinear saddle-point problems
//
//               Find ??? ﹋ H(div,次) and ??? ﹋ L2(次) such that
//               ( (?R)?1(???) , 而 ) + ( ??? , ??而 ) = 0                     ? 而 ﹋ H(div,次)
//               ( ????? , v )                     = ( ??????? - 汐? , v )    ? v ﹋ L2(次)
//
//              where (?R)?1(??) = ?? / ( 1 + |??|2 )^{1/2} and 汐? = 汐?r?, where r ≡ 1
...
//              with a tunable regularization parameter 0 ≒ ? << 1. The solver is
...
//                  Foundations of Computational Mathematics, 1每97.
...
int main(int argc, char *argv[])
{
   ...
   for (k = 0; k < max_it; k++)
   {
      ...
      if (myid == 0)
      {
         ...
         mfem::out << "Increment (|| u? - u?_prvs||) = " << increment_u << endl;
      }
      ...
      // Safeguard 2: Stop |肉| from growing too large
