...
//
//                      minimize ||?u||2 subject to u ¡Ý ? in H1?.
//
//              This is known as the obstacle problem, and it is a simple
//              mathematical model for contact mechanics.
//
//              In this example, the obstacle ? is a half-sphere centered
//              at the origin of a circular domain ¦¸. After solving to a
...
int main(int argc, char *argv[])
{
   ...
   *nodes /= scale;
   ...
   // 9. Initialize the slack variable ¦×? = ln(u?)
   ...
   for (k = 0; k < max_it; k++)
   {
      ...
      for ( j = 0; j < 10; j++)
      {
         ...
         if (order == 1)
         {
            // NOTE: ??u? = 0 for constant functions.
      ...
      mfem::out << "H1-error  (|| u - u??||)       = " << H1_error << endl;
...

   {
      ...
      mfem::out << "\n Final L2-error (|| u - u?||)          = " << L2_error <<
                endl;
      mfem::out << " Final H1-error (|| u - u?||)          = " << H1_error << endl;
      mfem::out << " Final L2-error (|| u - ? - exp(¦×?)||) = " << L2_error_alt <<
                endl;
   }
