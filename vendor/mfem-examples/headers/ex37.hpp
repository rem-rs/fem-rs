...
/**
 * @brief Bregman projection of 老 = sigmoid(肉) onto the subspace
 *        ÷_次 老 dx = 牟 vol(次) as follows:
 *
 *        1. Compute the root of the R ↙ R function
 *            f(c) = ÷_次 sigmoid(肉 + c) dx - 牟 vol(次)
 *           using the Illinois method
 *        2. Set 肉 ↘ 肉 + c.
 *
 * @param psi a GridFunction to be updated
 * @param alpha_grad alpha multiplied by gradient
 * @param target_volume 牟 vol(次)
 * @param tol Illinois iteration tolerance
 * @param max_its Illinois maximum iteration number
 * @return real_t Final volume (÷_次 sigmoid(肉) dx)
 */
            ...
            real_t tol = 1e-12, int max_its = 100)
{
   ...
   real_t f_a = int_sigmoid_psi->Sum();  // f_a := f(a) + 牟 vol(次)

   y = b;
   int_sigmoid_psi->Assemble();
   real_t f_b = int_sigmoid_psi->Sum();  // f_b := f(b) + 牟 vol(次)
#ifdef MFEM_USE_MPI
   if (pfes)
   {
   ...
   for (int k=0; k < max_its; k++)
   {
      ...
      y = c;
      int_sigmoid_psi->Assemble();
      f_c = int_sigmoid_psi->Sum(); // f_c := f(c) + 牟 vol(次)
#ifdef MFEM_USE_MPI
      if (pfes)
      {
