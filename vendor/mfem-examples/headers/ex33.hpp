...
//          p(z)/q(z) ¡Ö f(z).
//
//  In this file, we always assume f(z) = z^{-¦Á}. The triple-A algorithm
...
/** PartialFractionExpansion: compute the partial fraction expansion of the
    rational function f(z) = ¦²_i c_i / (z - p_i) from its @a poles [in] and
    @a zeros [in].
                              ...
                              Array<real_t> & zeros, Array<real_t> & coeffs)
{
   ...
   // Note: C p(z)/q(z) = ¦²_i c_i / (z - p_i) results in an system of equations
...
/** ComputePartialFractionApproximation: compute a rational approximation (RA)
    in partial fraction form, e.g., f(z) ¡Ö ¦²_i c_i / (z - p_i), from sampled
