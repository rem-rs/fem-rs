...
pub fn build_iga_gmg_hierarchy_3d(
    ...
    for (kv_u, kv_v, kv_w) in level_knots.iter() {
        ...
        levels.push(GeometricMgLevel { mat, bc_dofs, elem_op: None, raw_diag: vec![], raw_dinv: vec![] });
...
mod tests {
    ...
    fn iga_gmg_1d_converges_fewer_iterations() {
        ...
        let levels = vec![
            GeometricMgLevel { mat: fine_mat.clone(), bc_dofs: bc_fine.clone(), elem_op: None, raw_diag: vec![], raw_dinv: vec![] },
            GeometricMgLevel { mat: mid_mat, bc_dofs: bc_mid, elem_op: None, raw_diag: vec![], raw_dinv: vec![] },
            GeometricMgLevel { mat: coarse_mat, bc_dofs: bc_coarse, elem_op: None, raw_diag: vec![], raw_dinv: vec![] },
        ];
