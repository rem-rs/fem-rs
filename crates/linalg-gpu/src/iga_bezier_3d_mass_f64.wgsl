// IGA Bezier 3D mass assembly — f64 variant.
// Requires SHADER_F64.
// Supports p,q,r ≤ 2 (max n_local = 27). Uniform knot vectors (C = I).
//
// M[a][b] = ∫ R_a · R_b · |det J| dξ dη dζ
...
fn bern_val(p: u32, xi: f64, i: u32) -> f64 {
    if p == 1u { return select(1.0 - xi, xi, i == 1u); }
    else {
...
fn bern_der(p: u32, xi: f64, i: u32) -> f64 {
    if p == 1u { return select(-1.0, 1.0, i == 1u); }
    else {
...
fn gauss_pt_01(order: u32, i: u32) -> f64 {
    if order == 2u { return select(0.21132486540518708, 0.7886751345948129, i == 1u); }
    else {
...
fn gauss_wt_01(order: u32, i: u32) -> f64 {
    ...
    else { if i == 1u { return 4.0 / 9.0; } return 5.0 / 18.0; }
}
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    var Me: array<f64, 729>;
    for (var i = 0u; i < 729u; i++) { Me[i] = 0.0; }
    ...
    for (var qi = 0u; qi < params.quad_order; qi++) {
        ...
        for (var qj = 0u; qj < params.quad_order; qj++) {
            ...
            for (var qk = 0u; qk < params.quad_order; qk++) {
                ...
                var B: array<f64, 27>;
                var dB_u: array<f64, 27>;
                var dB_v: array<f64, 27>;
                var dB_w: array<f64, 27>;
                ...
                for (var a = 0u; a < n_loc; a++) {
                    let wa = elem.weights[a];
                    R[a] = wa * B[a] * invW;
                    dR_u[a] = (wa * dB_u[a] * W - wa * B[a] * dW_u) * invW2;
                    dR_v[a] = (wa * dB_v[a] * W - wa * B[a] * dW_v) * invW2;
                    dR_w[a] = (wa * dB_w[a] * W - wa * B[a] * dW_w) * invW2;
                }
                ...
                // 4. Mass: M[a][b] += R_a * R_b * |det J| * w
                let w = wx * wy * wz * abs(det_j);
                for (var a = 0u; a < n_loc; a++) {
                    let ra = R[a];
                    for (var b = 0u; b < n_loc; b++) {
                        Me[a * n_loc + b] += ra * R[b] * w;
                    }
                }
            }
        }
