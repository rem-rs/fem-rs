// IGA Bezier 2D mass assembly — f64 variant.
// Requires SHADER_F64.
// Supports p,q ≤ 3 (max n_local = 16). Uniform knot vectors (C = I).
//
// M[a][b] = ∫ R_a · R_b · |det J| dξ dη
...
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    var Me: array<f64, 256>;
    for (var i = 0u; i < 256u; i++) { Me[i] = 0.0; }
    ...
    for (var qi = 0u; qi < params.quad_order; qi++) {
        ...
        for (var qj = 0u; qj < params.quad_order; qj++) {
            ...
            // 1. Bernstein basis values
            var B: array<f64, 16>;
            var dB_u: array<f64, 16>;
            var dB_v: array<f64, 16>;
            ...
            // 4. Mass: M[a][b] += R_a * R_b * |det J| * wξ * wη
