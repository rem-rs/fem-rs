// Hex8 mass matrix — f64 variant. Requires SHADER_F64.
...
fn sv(xi:f64,eta:f64,ze:f64,sx:f64,sy:f64,sz:f64)->f64{
    return(1.0+sx*xi)*(1.0+sy*eta)*(1.0+sz*ze)/8.0;
}
fn det3(a00:f64,a01:f64,a02:f64,a10:f64,a11:f64,a12:f64,a20:f64,a21:f64,a22:f64)->f64{
    return a00*(a11*a22-a12*a21)-a01*(a10*a22-a12*a20)+a02*(a10*a21-a11*a20);
}
fn gx(xi:f64,eta:f64,ze:f64,sx:f64,sy:f64,sz:f64)->f64{return sx*(1.0+sy*eta)*(1.0+sz*ze)/8.0;}
fn gy(xi:f64,eta:f64,ze:f64,sx:f64,sy:f64,sz:f64)->f64{return sy*(1.0+sx*xi)*(1.0+sz*ze)/8.0;}
fn gz(xi:f64,eta:f64,ze:f64,sx:f64,sy:f64,sz:f64)->f64{return sz*(1.0+sx*xi)*(1.0+sy*eta)/8.0;}

@compute @workgroup_size(64)
fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
    let e=gid.x;if e>=params.n_elements{return;}
    let elem=elements[e];
    var Me:array<f64,64>;for(var i=0u;i<64u;i++){Me[i]=0.0;}
    ...
    for(var q=0u;q<8u;q++){
        ...
        for(var k=0u;k<8u;k++){
            let gx_=gx(xi,eta,ze,sx[k],sy[k],sz[k]);
            let gy_=gy(xi,eta,ze,sx[k],sy[k],sz[k]);
            let gz_=gz(xi,eta,ze,sx[k],sy[k],sz[k]);
            let nx=elem.nodes[3u*k];let ny=elem.nodes[3u*k+1u];let nz=elem.nodes[3u*k+2u];
            j00+=gx_*nx;j01+=gy_*nx;j02+=gz_*nx;
            j10+=gx_*ny;j11+=gy_*ny;j12+=gz_*ny;
            j20+=gx_*nz;j21+=gy_*nz;j22+=gz_*nz;
        }
        let dj=det3(j00,j01,j02,j10,j11,j12,j20,j21,j22);
        if dj<=0.0{continue;}
        var sv_:array<f64,8>;
        for(var k=0u;k<8u;k++){sv_[k]=sv(xi,eta,ze,sx[k],sy[k],sz[k]);}
        for(var i=0u;i<8u;i++){for(var j=0u;j<8u;j++){
            Me[i*8u+j]+=sv_[i]*sv_[j]*dj;
        }}
    }
    let b=e*64u;
    for(var i=0u;i<8u;i++){let di=elem.dofs[i];
    for(var j=0u;j<8u;j++){let idx=b+i*8u+j;
        coo_out[idx].row=di;coo_out[idx].col=elem.dofs[j];coo_out[idx].val=Me[i*8u+j];
    }}
}
