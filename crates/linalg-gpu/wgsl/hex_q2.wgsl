...
// DOFs in — MFEM's `H1_HexahedronElement(2)` order (D31 closed the former
// fem-rs legacy order; probe `tmp/d31/probe_h1_hex.cpp`).  Pinned against the
// Rust element layer by `pa_apply::tests::hex_q2_wgsl_slots_match_element`.
fn q2map(n:u32)->array<u32,3>{
 if(n<8u){return array<u32,3>(2u*((n&1u)^((n>>1u)&1u)),2u*((n>>1u)&1u),2u*(n>>2u));}
 let i=n-8u;
 return array<u32,3>(array<u32,19>(1,2,1,0,1,2,1,0,0,2,2,0,1,1,2,1,0,1,1)[i],
                     array<u32,19>(0,1,2,1,0,1,2,1,0,0,2,2,1,0,1,2,1,1,1)[i],
                     array<u32,19>(0,0,0,0,2,2,2,2,1,1,1,1,0,1,1,1,1,2,1)[i]);
}
