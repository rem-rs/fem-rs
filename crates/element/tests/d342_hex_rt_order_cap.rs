...
//!
//! ```text
//!     W[i][j] = phi_j(x_i) · nk_i = delta_ij   for every order k
//! ```
//!
//! (D721: on MFEM's `[0,1]³` frame the dual is the identity — the historical
//! `1/4` was the `[-1,1]` pull-back of two halved open factors, which the unit
//! cube's `cof(J) = (1/4)I` used to cancel in the physical dof).  A *global*
//! factor error in the basis cannot hide here — the diagonal is checked
//! against 1 itself, not against the basis — and the slot ordering, the
//! outward-normal signs and the D225 interior flips are all exercised because
//! any mismatch shows up as an off-diagonal entry.
//!
...
fn project_rt_rows(k: usize) -> (Vec<[f64; 3]>, Vec<[f64; 3]>) {
    ...
    let mut xs = Vec::new();
    let mut nks = Vec::new();
    // D721: MFEM's `[0,1]³` frame — open points on `[0,1]`, face coordinates
    // 0/1 (the probe `tmp/d721/probe_d721.cpp` dumps the same values).
...
/// D342/D721: the element must own the exact dual of MFEM's `Project_RT`
/// samples for every order the raised cap allows (0..=6), i.e. `W = I` on the
/// `[0,1]³` reference hex.  This is what makes
/// `HDivSpace::interpolate_vector`'s dense solve return MFEM's dof values
/// themselves.
#[test]
fn d342_hex_rt_project_rt_dual_matrix_is_identity() {
    for k in 0..=6usize {
        ...
        for i in 0..n {
            ...
            for j in 0..n {
                ...
                if i == j {
                    worst_diag = worst_diag.max((w - 1.0).abs());
                } else {
                    worst_off = worst_off.max(w.abs());
                }
            }
        }
        println!("  RT{k}: |W_ii - 1| <= {worst_diag:.3e}, |W_ij| <= {worst_off:.3e}");
        assert!(worst_diag < 1e-13, "k={k}: dual diagonal deviates from 1 by {worst_diag}");
...
fn d342_hex_rt_dof_coords_and_counts_orders_3_to_6() {
    for k in 3..=6usize {
        ...
        // Face slots: 6 faces x (k+1)^2, exactly one coordinate at 0 or 1.
        for (i, x) in c.iter().enumerate().take(6 * m * m) {
            let at_face = x.iter().filter(|v| **v == 0.0 || **v == 1.0).count();
        ...
        let glc: Vec<f64> = gauss_lobatto_arbitrary(k + 2)
            .0
            .iter()
            .map(|x| 0.5 * (x + 1.0))
            .collect();
        ...
        for (i, x) in interior.iter().enumerate() {
            assert!(x.iter().all(|v| *v > 0.0 && *v < 1.0), "k={k} interior slot {i}: {x:?}");
            let on_gll = x
                .iter()
                .filter(|v: &&f64| glc.iter().any(|g| (g - **v).abs() < 1e-14))
                .count();
