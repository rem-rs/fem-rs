...
mod tests {
    ...
    #[test]
    fn pyra_rtk_k1_dim() {
        assert_eq!(pyramid_rtk_dim(1), 17);
    }
    #[test]
    fn pyra_rtk_k2_dim() {
        assert_eq!(pyramid_rtk_dim(2), 39);
    }
    #[test]
    fn pyra_rtk_k3_dim() {
        assert_eq!(pyramid_rtk_dim(3), 74);
    }
    #[test]
    fn pyra_rtk_k0_basis_finite() {
        let e = PyraRTk::new(0);
    ...
    #[test]
    fn pyra_rtk_k1_basis_finite() {
        let e = PyraRTk::new(1);
        let mut v = vec![0.0; 51];
        for p in &e.quadrature(3).points {
            e.eval_basis_vec(p, &mut v);
            for x in &v {
                assert!(x.is_finite());
