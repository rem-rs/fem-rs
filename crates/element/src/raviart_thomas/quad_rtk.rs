//! Arbitrary-order RT_k on reference quadrilateral [-1,1]² via Vandermonde.
...
struct QuadRTkData { coeff: Vec<f64>, n: usize }

/// Gauss-Legendre nodes/weights on [-1,1]
fn gl_1d(n: usize) -> (Vec<f64>, Vec<f64>) {
    let mut x = vec![0.0; n]; let mut w = vec![0.0; n];
    for i in 0..n {
        let mut z = (std::f64::consts::PI * (i as f64 + 0.75) / (n as f64 + 0.5)).cos();
        for _ in 0..16 {
            let mut p1 = 1.0; let mut p2 = 0.0;
            for j in 1..=n { let p3 = p2; p2 = p1; p1 = ((2*j-1) as f64 * z * p2 - (j-1) as f64 * p3) / j as f64; }
            let pp = n as f64 * (z * p1 - p2) / (z * z - 1.0 + 1e-30);
            let dz = p1 / pp; z -= dz; if dz.abs() < 1e-14 { break; }
        }
        x[i] = z;
        let mut p1 = 1.0; let mut p2 = 0.0;
        for j in 1..=n { let p3 = p2; p2 = p1; p1 = ((2*j-1) as f64 * x[i] * p2 - (j-1) as f64 * p3) / j as f64; }
        w[i] = 2.0 / ((1.0 - x[i] * x[i]) * (n as f64 * p2 * p2));
    }
    (x, w)
}
...
fn quad_data(k: usize) -> &'static QuadRTkData {
    ...
    CACHE[k].get_or_init(|| {
        ...
        let mut qp2 = Vec::new(); let mut qw2 = Vec::new();
        for (&xi, &wi) in gx.iter().zip(gw.iter()) {
            for (&xj, &wj) in gx.iter().zip(gw.iter()) { qp2.push((xi, xj)); qw2.push(wi * wj); }
        }

        // Edge 0 (bottom): y=-1, n=(0,-1). ∫ -Φ_y(x,-1)·x^p dx
        for p in 0..=k {
        ...
        // Edge 3 (left): x=-1, n=(-1,0). ∫ -Φ_x(-1,y)·y^p dy
        for p in 0..=k {
            for j in 0..nx {
                let a = j / (k + 1); let b = j % (k + 1);
                let xv = (-1.0_f64).powi(a as i32); // x^a at x=-1
        ...
        for c in 0..n {
            ...
            row.swap(c, pr);
            let piv = row[c][c]; assert!(piv.abs() > 1e-14, "QuadRTk({k}) singular at col {c}");
            let ip = 1.0 / piv; for j in c..2 * n { row[c][j] *= ip; }
            for rr in 0..n { if rr != c { let f = row[rr][c]; for j in c..2 * n { row[rr][j] -= f * row[c][j]; } } }
        }
...
pub struct QuadRTk { order: usize }
impl QuadRTk {
    pub fn new(p: usize) -> Self { QuadRTk { order: p } }
}

impl VectorReferenceElement for QuadRTk {
    fn dim(&self) -> u8 { 2 }
    fn order(&self) -> u8 { self.order as u8 }
    fn n_dofs(&self) -> usize { 2 * (self.order + 1) * (self.order + 2) }

    fn eval_basis_vec(&self, xi: &[f64], values: &mut [f64]) {
        let k = self.order; let d = quad_data(k); let n = d.n;
    ...
    fn eval_div(&self, xi: &[f64], div_vals: &mut [f64]) {
        ...
        for a in 0..=k + 1 { for b in 0..=k {
            ...
            dm[idx] = (a as f64) * xm1 * y.powi(b as i32); idx += 1;
        }}
        // y-comp div: ∂/∂y (x^a y^b) = b·x^a y^(b-1)
        for a in 0..=k { for b in 0..=k + 1 {
            let ym1 = if b > 0 { y.powi((b - 1) as i32) } else { 0.0 };
            dm[idx] = (b as f64) * x.powi(a as i32) * ym1; idx += 1;
        }}
        for i in 0..n {
            let mut s = 0.0;
    ...
    fn dof_coords(&self) -> Vec<Vec<f64>> {
        let k = self.order; let n = self.n_dofs();
        let mut c = Vec::with_capacity(n);
        let step = 2.0 / (k + 2) as f64;
        for p in 0..=k { c.push(vec![-1.0 + step * (p + 1) as f64, -1.0]); }
        for p in 0..=k { c.push(vec![1.0, -1.0 + step * (p + 1) as f64]); }
        for p in 0..=k { c.push(vec![-1.0 + step * (p + 1) as f64, 1.0]); }
        for p in 0..=k { c.push(vec![-1.0, -1.0 + step * (p + 1) as f64]); }
        for _ in c.len()..n { c.push(vec![0.0, 0.0]); }
        c
    }
}
...
mod tests {
    use super::*;
    #[test] fn n_dofs() { assert_eq!(QuadRTk::new(0).n_dofs(), 4); assert_eq!(QuadRTk::new(1).n_dofs(), 12); assert_eq!(QuadRTk::new(2).n_dofs(), 24); assert_eq!(QuadRTk::new(3).n_dofs(), 40); }
    #[test] fn coeff_non_singular() { for k in 1..=4 { let d = quad_data(k); let n = d.n; let mut ok = true;
        for i in 0..n { let mut s = 0.0; for j in 0..n { s += d.coeff[j * n + i].abs(); } if s < 1e-10 { ok = false; } }
        assert!(ok, "k={k}"); } }
    #[test] fn finite() { for k in 0..=3 { let e = QuadRTk::new(k); let n = e.n_dofs(); let mut v = vec![0.0; n*2]; let mut d = vec![0.0; n];
        for p in &[(0.0,0.0),(0.3,-0.5),(-0.2,0.4)] { e.eval_basis_vec(&[p.0,p.1], &mut v); e.eval_div(&[p.0,p.1], &mut d);
            for &val in v.iter().chain(d.iter()) { assert!(val.is_finite(), "k={k} at ({},{})", p.0,p.1); } } } }
}
