...
fn tet_int(a: usize, b: usize, c: usize) -> f64 {
    (1..=a).fold(1.0, |p, i| p * i as f64)
        * (1..=b).fold(1.0, |p, i| p * i as f64)
...
fn tet_data(k: usize) -> &'static TetRTkData {
    static CACHE: [OnceLock<TetRTkData>; 9] = [
        OnceLock::new(),
        OnceLock::new(),
        OnceLock::new(),
        OnceLock::new(),
    ...
    CACHE[k].get_or_init(|| {
        ...
        for c in 0..n {
            ...
            row.swap(c, pr);
            let piv = row[c][c];
            assert!(piv.abs() > 1e-14, "TetRTk({k}) singular at col {c}");
            let ip = 1.0 / piv;
            for j in c..n + mt {
                row[c][j] *= ip;
            }
...
impl VectorReferenceElement for TetRTk {
    ...
    fn n_dofs(&self) -> usize {
        (self.order + 1) * (self.order + 2) * (self.order + 4) / 2
    }
        ...
        for dg in 0..=k {
            for a in 0..=dg {
                for b in 0..=(dg - a) {
                    ...
                    dm[idx] = (a as f64) * xm1 * yp * zp;
                    idx += 1;
                    dm[idx] = (b as f64) * xp * ym1 * zp;
                    idx += 1;
                    dm[idx] = (c as f64) * xp * yp * zm1;
                    idx += 1;
                }
            }
        }
    ...
    fn dof_coords(&self) -> Vec<Vec<f64>> {
        ...
        for _ in 0..face_dofs_per_face {
            c.push(vec![1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0]);
        }
        // Face 1: x=0, centroid (0, 1/3, 1/3)
        for _ in 0..face_dofs_per_face {
            c.push(vec![0.0, 1.0 / 3.0, 1.0 / 3.0]);
        }
        // Face 2: y=0, centroid (1/3, 0, 1/3)
        for _ in 0..face_dofs_per_face {
            c.push(vec![1.0 / 3.0, 0.0, 1.0 / 3.0]);
        }
        // Face 3: z=0, centroid (1/3, 1/3, 0)
        for _ in 0..face_dofs_per_face {
            c.push(vec![1.0 / 3.0, 1.0 / 3.0, 0.0]);
        }
        // Interior DOFs: scattered inside
        for i in 0..interior_dofs {
            let t = (i as f64 + 0.5) / interior_dofs as f64;
            c.push(vec![
                0.25 + 0.12 * (t * std::f64::consts::PI).cos(),
                0.25 + 0.12 * (t * std::f64::consts::PI * 1.3).sin(),
                0.25 + 0.12 * (t * std::f64::consts::PI * 0.7).cos(),
            ]);
        }
        c
    }
