...
fn quad_data(k: usize) -> &'static QuadBDMkData {
    CACHE[k].get_or_init(|| {
        ...
        QuadBDMkData { k, n: n_edge + n_int, n_interior: n_int }
    })
}

pub struct QuadBDMk { order: usize }

impl QuadBDMk {
    pub fn new(p: usize) -> Self { assert!(p >= 1); QuadBDMk { order: p } }
    ...
    fn lagrange(&self, i: usize, t: f64) -> f64 {
        ...
        for j in 0..=k { if j == i { continue; }
            num *= t - nodes[j]; den *= nodes[i] - nodes[j];
        }
        num / den
    }
    fn dlagrange(&self, i: usize, t: f64) -> f64 {
        ...
        for j in 0..=k { if j == i { continue; }
            let mut num = 1.0; let mut den = 1.0;
            for jj in 0..=k { if jj == i || jj == j { continue; }
                num *= t - nodes[jj]; den *= nodes[i] - nodes[jj];
            }
            sum += num / den;
        }
        sum
...
impl VectorReferenceElement for QuadBDMk {
    fn n_dofs(&self) -> usize { quad_data(self.order).n }
    fn dim(&self) -> u8 { 2u8 }
    ...
    fn eval_basis_vec(&self, xi: &[f64], vals: &mut [f64]) {
        ...
        for i in 0..=k { if idx < n { vals[idx*2] = 0.0; vals[idx*2+1] = -self.lagrange(i, x); idx += 1; } }
        // Edge 1: x = 1, normal (1, 0) → comp (L_i(y), 0)
        for i in 0..=k { if idx < n { vals[idx*2] = self.lagrange(i, y); vals[idx*2+1] = 0.0; idx += 1; } }
        ...
        if k >= 2 {
            for p in 0..k-1 { for q in 0..k-1 {
                if idx < n { let b = (1.0-x*x)*(1.0-y*y);
                    vals[idx*2] = b * x.powi(p as i32) * y.powi(q as i32);
                    vals[idx*2+1] = b * x.powi(p as i32) * y.powi(q as i32);
                    idx += 1; }
            }}
        }
        while idx < n { vals[idx*2] = 0.0; vals[idx*2+1] = 0.0; idx += 1; }
    }
    ...
    fn eval_div(&self, xi: &[f64], div: &mut [f64]) {
        ...
        let (x, y) = (xi[0], xi[1]);
        let mut idx = 0;
        for _ in 0..4*(k+1) { if idx < n { div[idx] = 0.0; idx += 1; } }
        if k >= 2 {
            for p in 0..k-1 { for q in 0..k-1 {
                if idx < n {
                    let b = (1.0-x*x)*(1.0-y*y);
                    let dx_b = -2.0*x*(1.0-y*y); let dy_b = -2.0*y*(1.0-x*x);
                    let mx = x.powi(p as i32); let my = y.powi(q as i32);
                    let dmx = if p>0 { p as f64 * x.powi(p as i32-1) } else { 0.0 };
                    let dmy = if q>0 { q as f64 * y.powi(q as i32-1) } else { 0.0 };
                    div[idx] = (dx_b*mx*my + b*dmx*my) + (dy_b*mx*my + b*mx*dmy);
                    idx += 1;
                }
            }}
        }
        while idx < n { div[idx] = 0.0; idx += 1; }
    }
    ...
    fn eval_curl(&self, xi: &[f64], curl: &mut [f64]) {
        ...
        for i in 0..=k { if idx < n { curl[idx] = -self.dlagrange(i, x); idx += 1; } }
        // Edge 1: comp=(L_i(y),0), curl=∂(0)/∂x - ∂(L_i(y))/∂y = -L_i'(y)
        for i in 0..=k { if idx < n { curl[idx] = -self.dlagrange(i, y); idx += 1; } }
        ...
        if k >= 2 {
            for p in 0..k-1 { for q in 0..k-1 {
                if idx < n {
                    ...
                    let dmx = if p>0 { p as f64 * x.powi(p as i32-1) } else { 0.0 };
                    let dmy = if q>0 { q as f64 * y.powi(q as i32-1) } else { 0.0 };
                    curl[idx] = (dx_b*mx*my + b*dmx*my) - (dy_b*mx*my + b*mx*dmy);
                    idx += 1;
                }
            }}
        }
        while idx < n { curl[idx] = 0.0; idx += 1; }
    }

    fn dof_coords(&self) -> Vec<Vec<f64>> {
        let d = quad_data(self.order);
        let k = d.k; let n = d.n;
        let nodes = if k >= 1 { let (p,_) = crate::quadrature::gauss_lobatto_arbitrary(k+2); p[..=k].to_vec() } else { vec![] };
        let mut coords = Vec::with_capacity(n);
        // Edge 0: y=-1
        for i in 0..=k { coords.push(vec![nodes[i], -1.0]); }
        // Edge 1: x=1
        for i in 0..=k { coords.push(vec![1.0, nodes[i]]); }
        // Edge 2: y=1
        for i in 0..=k { coords.push(vec![nodes[i], 1.0]); }
        // Edge 3: x=-1
        for i in 0..=k { coords.push(vec![-1.0, nodes[i]]); }
        ...
        for p in 0..k-1 { for q in 0..k-1 {
            coords.push(vec![2.0*p as f64/(k-1)as f64-1.0, 2.0*q as f64/(k-1)as f64-1.0]);
        }}
        while coords.len() < n { coords.push(vec![0.0, 0.0]); }
        coords
    }
}
...
mod tests {
    ...
    #[test]
    fn quad_bdmk_finite() {
        for k in 1..=3 { let e = QuadBDMk::new(k); let n = e.n_dofs();
            let mut v = vec![0.0; n*2]; let mut d = vec![0.0; n];
