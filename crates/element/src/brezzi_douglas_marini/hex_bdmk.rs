            ...
            for midx in 0..m {
                ...
                for (pt, &w) in g3_pts.iter().zip(g3_wts.iter()) {
                    let div_mv = match monos[midx].comp {
                        0 => {
                            let a = monos[midx].a as f64;
                            if a > 0.0 {
                                a * pt[0].powi(monos[midx].a as i32 - 1)
                                    * pt[1].powi(monos[midx].b as i32)
                                    * pt[2].powi(monos[midx].c as i32)
                            } else {
                                0.0
                            }
                        }
                        1 => {
                            let b = monos[midx].b as f64;
                            if b > 0.0 {
                                b * pt[0].powi(monos[midx].a as i32)
                        ...
                        2 => {
                            let c = monos[midx].c as f64;
                            if c > 0.0 {
                                c * pt[0].powi(monos[midx].a as i32)
                                    * pt[1].powi(monos[midx].b as i32)
                                    * pt[2].powi(monos[midx].c as i32 - 1)
                            } else {
                                0.0
                            }
                        }
...
impl VectorReferenceElement for HexBDMk {
    ...
    fn dof_coords(&self) -> Vec<Vec<f64>> {
        ...
        let mut c = Vec::new();
        for _ in 0..n_mom {
            c.push(vec![0.0, 0.3, 0.3]);
        }
        for _ in 0..n_mom {
            c.push(vec![1.0, 0.3, 0.3]);
        }
        for _ in 0..n_mom {
            c.push(vec![0.3, 0.0, 0.3]);
        }
        for _ in 0..n_mom {
            c.push(vec![0.3, 1.0, 0.3]);
        }
        for _ in 0..n_mom {
            c.push(vec![0.3, 0.3, 0.0]);
        }
        for _ in 0..n_mom {
            c.push(vec![0.3, 0.3, 1.0]);
        }
        c
    }
}
...
mod tests {
    ...
    #[test]
    fn hex_bdmk_k1_dim() {
        assert_eq!(hex_bdmk_dim(1), 18);
    }
    #[test]
    fn hex_bdmk_k2_dim() {
        assert_eq!(hex_bdmk_dim(2), 39);
    }
    #[test]
    fn hex_bdmk_k3_dim() {
        assert_eq!(hex_bdmk_dim(3), 72);
    }

    #[test]
    fn hex_bdmk_k1_basis_finite() {
        let e = HexBDMk::new(1);
        let mut v = vec![0.0; 54];
        for p in &e.quadrature(2).points {
    ...
    #[test]
    fn hex_bdmk_k2_basis_finite() {
        let e = HexBDMk::new(2);
        let mut v = vec![0.0; 117];
        for p in &e.quadrature(2).points {
            e.eval_basis_vec(p, &mut v);
            for x in &v {
                assert!(x.is_finite());
