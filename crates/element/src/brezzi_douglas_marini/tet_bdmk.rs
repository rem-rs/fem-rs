...
fn tet_data(k: usize) -> &'static TetBDMkData {
    static CACHE: [OnceLock<TetBDMkData>; 9] = [
        OnceLock::new(),
        OnceLock::new(),
        OnceLock::new(),
        OnceLock::new(),
    ...
    CACHE[k].get_or_init(|| {
            ...
            for b in 0..=(k - a) {
                for (j, &(am, bm, cm, _comp)) in mc.iter().enumerate() {
                    ...
                    for (qp, w) in qr.points.iter().zip(qr.weights.iter()) {
                        ...
                        let z = v;
                        s += w
                            * (x.powi(am as i32) * y.powi(bm as i32) * z.powi(cm as i32))
                            * u.powi(a as i32)
                            * v.powi(b as i32);
                    }
                    v[di][j] = s;
        ...
        for c in 0..n {
            ...
            row.swap(c, pr);
            let piv = row[c][c];
            assert!(piv.abs() > 1e-14, "TetBDMk({k}) singular at col {c}");
            let ip = 1.0 / piv;
            for j in c..n + mt {
                row[c][j] *= ip;
            }
...
impl VectorReferenceElement for TetBDMk {
    ...
    fn n_dofs(&self) -> usize {
        (self.order + 1) * (self.order + 2) * (self.order + 3) / 2
    }
