...
impl VectorReferenceElement for HexND1 {
    ...
    fn eval_basis_vec(&self, xi: &[f64], values: &mut [f64]) {
        ...
        //   e8..e11 z-edges (0,4),(1,5),(2,6),(3,7).
        values[0] = 0.125 * (1.0 - y) * (1.0 - z); // e0 (x, y=-1, z=-1)
        values[1] = 0.0;
        values[2] = 0.0;
        ...
        values[6] = 0.125 * (1.0 + y) * (1.0 - z); // e2 (x, y=+1, z=-1)
        values[7] = 0.0;
        values[8] = 0.0;
        ...
        values[12] = 0.125 * (1.0 - y) * (1.0 + z); // e4 (x, y=-1, z=+1)
        values[13] = 0.0;
        values[14] = 0.0;
        ...
        values[18] = 0.125 * (1.0 + y) * (1.0 + z); // e6 (x, y=+1, z=+1)
        values[19] = 0.0;
        values[20] = 0.0;
    ...
    fn dof_coords(&self) -> Vec<Vec<f64>> {
        vec![
            // MFEM CUBE Edges ordering: (0,1),(1,2),(3,2),(0,3),(4,5),(5,6),
            // (7,6),(4,7),(0,4),(1,5),(2,6),(3,7).
            vec![0.0, -1.0, -1.0], // e0 (0,1) x
            vec![1.0, 0.0, -1.0],  // e1 (1,2) y
            vec![0.0, 1.0, -1.0],  // e2 (3,2) x
            vec![-1.0, 0.0, -1.0], // e3 (0,3) y
            vec![0.0, -1.0, 1.0],  // e4 (4,5) x
            vec![1.0, 0.0, 1.0],   // e5 (5,6) y
            vec![0.0, 1.0, 1.0],   // e6 (7,6) x
            vec![-1.0, 0.0, 1.0],  // e7 (4,7) y
            vec![-1.0, -1.0, 0.0], // e8 (0,4) z
            vec![1.0, -1.0, 0.0],  // e9 (1,5) z
            vec![1.0, 1.0, 0.0],   // e10 (2,6) z
            vec![-1.0, 1.0, 0.0],  // e11 (3,7) z
        ]
    }
}
...
mod tests {
    ...
    fn nd1_hex_edge_moments_are_nodal() {
        ...
        let tangents = [
            [1.0, 0.0, 0.0], // e0 (0,1) x
            [0.0, 1.0, 0.0], // e1 (1,2) y
            [1.0, 0.0, 0.0], // e2 (3,2) x
            [0.0, 1.0, 0.0], // e3 (0,3) y
            [1.0, 0.0, 0.0], // e4 (4,5) x
            [0.0, 1.0, 0.0], // e5 (5,6) y
            [1.0, 0.0, 0.0], // e6 (7,6) x
            [0.0, 1.0, 0.0], // e7 (4,7) y
            [0.0, 0.0, 1.0], // e8 (0,4) z
            [0.0, 0.0, 1.0], // e9 (1,5) z
            [0.0, 0.0, 1.0], // e10 (2,6) z
            [0.0, 0.0, 1.0], // e11 (3,7) z
        ];
