...
impl VectorReferenceElement for HexNDk {
    ...
    fn eval_basis_vec(&self, xi: &[f64], values: &mut [f64]) {
        let p=self.order; let nd=self.nodes();
        ...
        if p >= 2 {
            ...
            for i in 0..=(p-2) { let yi=y.powi(i as i32); let by=1.0-y*y;
                for l in 0..=(p-2) { let zl=z.powi(l as i32); let bz=1.0-z*z;
                    for j in 0..p { values[off*3] = lag(&nd,j,x)*by*yi*bz*zl; off+=1; } } }
            // y-comp: (1-x²)(1-z²)·x^i·l_j(y)·z^l, i,l=0..k-2, j=0..k-1
            for i in 0..=(p-2) { let xi=x.powi(i as i32); let bx=1.0-x*x;
                for l in 0..=(p-2) { let zl=z.powi(l as i32); let bz=1.0-z*z;
                    for j in 0..p { values[off*3+1] = lag(&nd,j,y)*bx*xi*bz*zl; off+=1; } } }
            // z-comp: (1-x²)(1-y²)·x^i·y^l·l_j(z), i,l=0..k-2, j=0..k-1
            for i in 0..=(p-2) { let xi=x.powi(i as i32); let bx=1.0-x*x;
                for l in 0..=(p-2) { let yl=y.powi(l as i32); let by=1.0-y*y;
                    for j in 0..p { values[off*3+2] = lag(&nd,j,z)*bx*xi*by*yl; off+=1; } } }
        }
    }
    ...
    fn eval_curl(&self, xi: &[f64], curl_vals: &mut [f64]) {
        ...
        if p >= 2 {
            ...
            for i in 0..=(p-2) { let yi=y.powi(i as i32); let ym=1.0-y*y;
                let dyi=if i>0{ i as f64*y.powi(i as i32-1) }else{ 0.0 };
                let hz=hat(z,-1.0); let dhz=hat_d(z,-1.0);
                for j in 0..p { let d=off; let lx=lag(&nd,j,x);
                    curl_vals[d*3+1]=lx*ym*yi*dhz;
                    curl_vals[d*3+2]=-lx*(-2.0*y*yi+ym*dyi)*hz;
                    off+=1; } }
            // z=-1 y-tangent: Φ=(0,ψ,0), curl=(-∂ψ/∂z, 0, ∂ψ/∂x)
            for i in 0..=(p-2) { let xi=x.powi(i as i32); let xm=1.0-x*x;
                let dxi=if i>0{ i as f64*x.powi(i as i32-1) }else{ 0.0 };
                let hz=hat(z,-1.0); let dhz=hat_d(z,-1.0);
                for j in 0..p { let d=off; let ly=lag(&nd,j,y);
            ...
            for i in 0..=(p-2) { let zi=z.powi(i as i32); let zm=1.0-z*z;
                let dzi=if i>0{ i as f64*z.powi(i as i32-1) }else{ 0.0 };
                let hy=hat(y,-1.0); let dhy=hat_d(y,-1.0);
                for j in 0..p { let d=off; let lx=lag(&nd,j,x);
            ...
            for i in 0..=(p-2) { let xi=x.powi(i as i32); let xm=1.0-x*x;
                let dxi=if i>0{ i as f64*x.powi(i as i32-1) }else{ 0.0 };
                let hy=hat(y,-1.0); let dhy=hat_d(y,-1.0);
                for j in 0..p { let d=off; let lz=lag(&nd,j,z);
            ...
            // curl=(-∂ψ/∂z, 0, ∂ψ/∂x)
            for i in 0..=(p-2) { let yi=y.powi(i as i32); let ym=1.0-y*y;
                let hx=hat(x,-1.0); let dhx=hat_d(x,-1.0);
                for j in 0..p { let d=off;
            ...
            for i in 0..=(p-2) { let yi=y.powi(i as i32); let by=1.0-y*y;
                let dyi=if i>0{ i as f64*y.powi(i as i32-1) }else{ 0.0 };
                for l in 0..=(p-2) { let zl=z.powi(l as i32); let bz=1.0-z*z;
                    let dzl=if l>0{ l as f64*z.powi(l as i32-1) }else{ 0.0 };
                    for j in 0..p { let d=off; let lx=lag(&nd,j,x);
            ...
            for i in 0..=(p-2) { let xi=x.powi(i as i32); let bx=1.0-x*x;
                let dxi=if i>0{ i as f64*x.powi(i as i32-1) }else{ 0.0 };
                for l in 0..=(p-2) { let yl=y.powi(l as i32); let by=1.0-y*y;
                    let dyl=if l>0{ l as f64*y.powi(l as i32-1) }else{ 0.0 };
                    for j in 0..p { let d=off; let lz=lag(&nd,j,z);
    ...
    fn dof_coords(&self) -> Vec<Vec<f64>> {
        ...
        if p >= 2 {
            ...
            for &zs in &[-1.0, 1.0] {
                for _ in 0..2 { // x-tangent, y-tangent
                    for i in 0..=(p-2) { let v = bp(i);
                        for j in 0..p { let u = nd[j];
                            c.push(vec![u, v, zs]); } } } }
            // y-faces: 4 groups × p(p-1) each
            for &ys in &[-1.0, 1.0] {
                for _ in 0..2 { // x-tangent, z-tangent
                    for i in 0..=(p-2) { let v = bp(i);
                        for j in 0..p { let u = nd[j];
                            c.push(vec![u, ys, v]); } } } }
            // x-faces: 4 groups × p(p-1) each
            for &xs in &[-1.0, 1.0] {
                for _ in 0..2 { // y-tangent, z-tangent
                    for i in 0..=(p-2) { let v = bp(i);
                        for j in 0..p { let u = nd[j];
                            c.push(vec![xs, u, v]); } } } }
            ...
            for _ in 0..3 {
                for i in 0..=(p-2) { for l in 0..=(p-2) { for j in 0..p {
                    c.push(vec![nd[j], bp2(i), bp2(l)]);
                } } }
            }
        }
        while c.len() < n { c.push(vec![0.0;3]); }
        c
    }
}
...
mod tests {
    ...
    #[test] fn finite() { for k in 1..=4 { let e=HexNDk::new(k); let n=e.n_dofs(); let mut v=vec![0.0;n*3];
        for p in &[(0.3,-0.5,0.7),(0.0,0.0,0.0),(-0.8,0.2,0.9)] { e.eval_basis_vec(&[p.0,p.1,p.2],&mut v);
            for &x in &v { assert!(x.is_finite(),"eval_basis k={k} at {p:?}"); } } } }
    #[test] fn curl_finite() { for k in 1..=4 { let e=HexNDk::new(k); let n=e.n_dofs(); let mut c=vec![0.0;n*3];
        for p in &[(0.3,-0.5,0.7),(0.0,0.0,0.0),(-0.8,0.2,0.9)] { e.eval_curl(&[p.0,p.1,p.2],&mut c);
            for &x in &c { assert!(x.is_finite(),"eval_curl k={k} at {p:?}"); } } } }
    #[test] fn dof_coords_count() { for k in 1..=4 {
        assert_eq!(HexNDk::new(k).dof_coords().len(), HexNDk::new(k).n_dofs()); } }
    #[test] fn edge_interp() {
        let e = HexNDk::new(2); let coords = e.dof_coords(); let n = 54; let mut v = vec![0.0; n*3];
        for (i, cd) in coords.iter().enumerate() {
            if i >= 24 { break; } // edge DOFs only (12k = 24 for k=2)
            e.eval_basis_vec(&[cd[0], cd[1], cd[2]], &mut v);
            let comp = if i < 8 { 0 } else if i < 16 { 1 } else { 2 };
            let val = v[i*3 + comp];
