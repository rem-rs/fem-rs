...
fn tet_data(k: usize) -> &'static TetNDkData {
    ...
    CACHE[k - 1].get_or_init(|| {
        ...
        for c in 0..n {
            ...
            r.swap(c, pr);
            let piv = r[c][c];
            assert!(piv.abs() > 1e-14, "TetNDk({k}) singular at col {c}");
            let ip = 1.0 / piv;
            for j in c..n + mt {
                r[c][j] *= ip;
            }
...
impl VectorReferenceElement for TetNDk {
    ...
    fn eval_curl(&self, xi: &[f64], curl_vals: &mut [f64]) {
        ...
        for (ji, &s) in d.monomap.iter().enumerate() {
            ...
            cm[ji * 3 + 1] = match comp {
                0 => fc * xp * yp * zm1,
                1 => 0.0,
                _ => -fa * xm1 * yp * zp,
            };
            cm[ji * 3 + 2] = match comp {
                0 => -fb * xp * ym1 * zp,
                1 => fa * xm1 * yp * zp,
                _ => 0.0,
            };
