...
impl EventFunction {
    pub fn new(name: &'static str, eval: fn(f64, &[f64]) -> f64) -> Self {
        EventFunction { name, eval, terminal: false, direction: 0, last_value: None, triggered: false }
    }
    pub fn terminal(mut self, v: bool) -> Self { self.terminal = v; self }
    pub fn direction(mut self, d: i8) -> Self { self.direction = d; self }
}
...
where G: Fn(f64) -> f64 {
    let mut a = t0; let mut b = t1;
    let mut ga = g0; let mut gb = g1;
    let mut side = 0i8;
    for _ in 0..max_iter {
        if (b - a).abs() < tol { return (a + b) / 2.0; }
        if ga.abs() < tol { return a; }
        if gb.abs() < tol { return b; }
        let t = if side == 0 { (a + b) / 2.0 } else { (a * gb - b * ga) / (gb - ga) };
        let t = t.clamp(a.min(b), a.max(b));
        let gt = g(t);
        if gt == 0.0 || (b - a).abs() < tol { return t; }
        if gt * gb > 0.0 { b = t; gb = gt; if side == -1 { ga *= 0.5; } side = -1; }
        else { a = t; ga = gt; if side == 1 { gb *= 0.5; } side = 1; }
    }
    (a + b) / 2.0
}
    ...
    F: Fn(f64, &[f64], &mut [f64]),
{
    ...
    use crate::butcher::{dopri5_tableau, wrms_error, i_step_controller};
    ...
    while t < t_end {
        if events_fired.iter().any(|e| e.terminal) { break; }

        let dt_step = dt.min(t_end - t);

        let (u_new, u_err, k) = explicit_adaptive_step(&rhs, &tableau, t, &u, dt_step);

        let err = if u_err.iter().any(|&e| e.is_nan()) { 1e20 }
        else { wrms_error(&u_new, &u_err, config.atol, config.rtol) };
        ...
        if err <= 1.0 {
            ...
            if !events.is_empty() {
                ...
                for ev in events.iter_mut() {
                    ...
                    let crossed = if ev.direction == 0 { g_old * g_new < 0.0 }
                        else if ev.direction > 0 { g_old < 0.0 && g_new >= 0.0 }
                        else { g_old > 0.0 && g_new <= 0.0 };
                    ...
                    if crossed {
                        ...
                        let g_wrap = |theta: f64| -> f64 {
                            if theta <= 0.0 { return g_old; }
                            if theta >= 1.0 { return g_new; }
                            ...
                            for j in 0..k.len() {
                                let bj = tableau.b()[j];
                                if bj.abs() > 1e-15 {
                                    for d in 0..n { y_interp[d] += hh * bj * k[j][d]; }
                                }
                            }
                            for d in 0..n { y_interp[d] = u[d] + theta * y_interp[d]; }
                            (ev.eval)(t + theta * hh, &y_interp)
                        };

                        let t_event = find_root_illinois(
                            g_wrap, 0.0, 1.0, g_old, g_new, 1e-12, 50,
                        );
                        ...
                        for j in 0..k.len() {
                            let bj = tableau.b()[j];
                            if bj.abs() > 1e-15 {
                                for d in 0..n { y_event[d] += dt_step * bj * k[j][d]; }
                            }
                        }
                        for d in 0..n { y_event[d] = u[d] + theta * y_event[d]; }
...
mod tests {
    ...
    fn decay_rhs(_t: f64, u: &[f64], dudt: &mut [f64]) { dudt[0] = -u[0]; }
    fn event_u_half(_t: f64, u: &[f64]) -> f64 { u[0] - 0.5 }
    fn event_t_03(t: f64, _y: &[f64]) -> f64 { t - 0.3 }
    ...
    fn integrate_stops_at_terminal_event() {
        ...
        let mut events = vec![
            EventFunction::new("u=0.5", event_u_half).terminal(true),
        ];
        let (u_final, events_fired) = integrate_with_events(
            decay_rhs, 0.0, 5.0, &u0, 0.1, &config, &mut events,
        );
        assert!(!events_fired.is_empty(), "event should fire");
        assert!((u_final[0] - 0.5).abs() < 0.01,
            "final u={}, expected≈0.5", u_final[0]);
    }
    ...
    fn multiple_events_both_fire() {
        ...
        let (_u_final, events_fired) = integrate_with_events(
            decay_rhs, 0.0, 1.0, &u0, 0.1, &config, &mut events,
        );
        assert_eq!(events_fired.len(), 2, "both events should fire");
        if events_fired.len() >= 2 {
            assert!(events_fired[0].t_event <= events_fired[1].t_event, "events out of order");
        }
    }
    ...
    fn no_events_no_effect() {
        ...
        let config = AdaptiveConfig::default();
        let (u_final, events_fired) = integrate_with_events(
            decay_rhs, 0.0, 1.0, &u0, 0.1, &config, &mut vec![],
        );
    ...
    fn illinois_root_finds_exact() {
        let root = find_root_illinois(
            |t| (-t).exp() - 0.5, 0.0, 2.0, 1.0 - 0.5, (-2.0_f64).exp() - 0.5, 1e-12, 50,
        );
