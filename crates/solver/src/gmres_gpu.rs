...
impl<T: Scalar> GmresGpuWorkspace<T> {
    ...
    pub fn measure_fixed_iters_arnoldi_only(
        &mut self,
        ctx: &GpuContext,
        x: &mut [T],
        cfg: &SolverConfig,
    ...
    pub fn measure_fixed_iters_spmv_only(
        &mut self,
        ctx: &GpuContext,
        x: &mut [T],
        cfg: &SolverConfig,
    ...
    fn solve_gmres(
            ...
            for j in 0..m {
                ...
                for i in 0..=j {
                    ...
                    let mut enc =
                        ctx.device
                            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                                label: Some("gmres_mgs"),
                            });
                    self.base.vops.encode_axpy(
                        ctx,
                        &mut enc,
                        -h[i * m + j],
                        &self.basis[i],
                ...
                if !fixed_iters {
                    ...
                    if r_norm_j <= tol || iter_count >= max_iter as u32 {
                        ...
                        _x.copy_from_slice(&self.base.read_solution(ctx));
                        return Ok(SolveResult {
                            converged: r_norm_j <= tol,
                            iterations: iter_count as usize,
                            final_residual: r_norm_j,
                        });
                    }
                }
            }

            if iter_count >= max_iter as u32 {
                break 'outer;
            }
        ...
        _x.copy_from_slice(&self.base.read_solution(ctx));
        let final_res = if fixed_iters {
            self.base.norm2(ctx, &self.base.gpu_r)
        } else {
            r_norm
        };
        Ok(SolveResult {
    ...
    fn measure(
        ...
        while iter_count < target {
            ...
            let mut enc = ctx
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("gmres_measure_restart"),
                });
            self.base.compute_residual(ctx, &mut enc);
            ctx.queue.submit([enc.finish()]);
            let r_norm = self.base.norm2(ctx, &self.base.gpu_r);
            let mut enc = ctx
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("gmres_measure_reseed"),
                });
            self.base.vops.encode_axpy(
                ctx,
                &mut enc,
        ...
        if !skip_update {
            ...
            let mut enc = ctx
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("gmres_measure_solution"),
                });
        ...
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("gmres_measure_final"),
            });
        self.base.compute_residual(ctx, &mut enc);
        ctx.queue.submit([enc.finish()]);
    ...
    fn profile(&mut self, ctx: &GpuContext, x: &mut [T], cfg: &SolverConfig) -> GmresGpuProfile {
        ...
        while iter_count < target {
            ...
            let mut enc = ctx
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("gmres_profile_restart"),
                });
            self.base.compute_residual(ctx, &mut enc);
            ctx.queue.submit([enc.finish()]);
            let r_norm = self.base.norm2(ctx, &self.base.gpu_r);
            let mut enc = ctx
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("gmres_profile_reseed"),
                });
            self.base.vops.encode_axpy(
                ctx,
                &mut enc,
        ...
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("gmres_profile_solution"),
            });
        for i in 0..n_mini {
            self.base.vops.encode_axpy(
                ctx,
        ...
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("gmres_profile_final"),
            });
        self.base.compute_residual(ctx, &mut enc);
        ctx.queue.submit([enc.finish()]);
        prof.final_residual = self.base.norm2(ctx, &self.base.gpu_r);
