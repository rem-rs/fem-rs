...
use fem_linalg_gpu::{DeviceBuffer, GpuContext, GpuJacobiPrecond, GpuVector};
use wgpu;
...
impl<T: Scalar> CgGpuWorkspace<T> {
    pub fn new(ctx: &GpuContext, a: &CsrMatrix<T>, b: &[T]) -> Self {
        ...
        Self {
            ...
            gpu_ap: GpuVector::zeros(ctx, n),
            gpu_tmp: GpuVector::zeros(ctx, n),
        }
    }
    ...
    fn solve_cg_prepared(
        &mut self,
        ctx: &GpuContext,
        cfg: &SolverConfig,
        fixed_iters: bool,
    ) -> (Result<SolveResult, SolverError>, Vec<T>) {
        ...
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("cg_init_p"),
            });
        self.base
            .vops
            .encode_axpy(ctx, &mut enc, 1.0, &self.base.gpu_r, 0.0, &self.gpu_p);
        ctx.queue.submit([enc.finish()]);

        let mut rsold = self
            .base
            .dot_readback(ctx, &self.base.gpu_r, &self.base.gpu_r);
        let mut iters = 0u32;
        ...
        for _ in 0..max_iter {
            ...
            self.base
                .vops
                .encode_axpy(ctx, &mut enc, -alpha, &self.gpu_tmp, 1.0, &self.base.gpu_r);
            ctx.queue.submit([enc.finish()]);
...
impl<T: Scalar> PcgGpuWorkspace<T> {
    ...
    fn solve_pcg_prepared(
        ...
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("pcg_apply_z"),
            });
        self.precond
            .encode_apply(ctx, &mut enc, &self.base.gpu_r, &mut self.gpu_z);
        ctx.queue.submit([enc.finish()]);
        ...
        let mut enc = ctx
            .device
            .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                label: Some("pcg_init_p"),
            });
        self.base
            .vops
            .encode_axpy(ctx, &mut enc, 1.0, &self.gpu_z, 0.0, &self.gpu_p);
        ctx.queue.submit([enc.finish()]);
        ...
        for _ in 0..max_iter {
            ...
            self.base
                .vops
                .encode_axpy(ctx, &mut enc, -alpha, &self.gpu_ap, 1.0, &self.base.gpu_r);
            ctx.queue.submit([enc.finish()]);

            let mut enc = ctx
                .device
                .create_command_encoder(&wgpu::CommandEncoderDescriptor {
                    label: Some("pcg_apply_z2"),
                });
            self.precond
                .encode_apply(ctx, &mut enc, &self.base.gpu_r, &mut self.gpu_z);
            ctx.queue.submit([enc.finish()]);
            ...
            self.base
                .vops
                .encode_axpy(ctx, &mut enc, 1.0, &self.gpu_z, 1.0, &self.gpu_p);
            ctx.queue.submit([enc.finish()]);
        }
