...
impl<T: Scalar> BicgstabGpuWorkspace<T> {
    ...
    fn solve_bicgstab(&mut self, ctx: &GpuContext, _x: &mut [T], cfg: &SolverConfig, fixed_iters: bool) -> Result<SolveResult, SolverError> {
        let max_iter = cfg.max_iter;
        let tol = (cfg.rtol.max(1e-30) * self.base.b_norm).max(cfg.atol.max(1e-30));

        // r₀ = b − A·x₀
        let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("bicgstab_init") });
        ...
        let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("bicgstab_r0") });
        self.base.vops.encode_axpy(ctx, &mut enc, 1.0, &self.base.gpu_r, 0.0, &self.gpu_r0);
        ctx.queue.submit([enc.finish()]);
        ...
        for _ in 0..max_iter {
            ...
            // Step 3: p = p + r      (add r)
            let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("bicgstab_p") });
            self.base.vops.encode_axpy(ctx, &mut enc, -omega, &self.gpu_v, 1.0, &self.gpu_p);
            ...
            // v = A·p
            let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("bicgstab_spmv") });
            self.base.spmv.encode_spmv(ctx, &mut enc, 1.0, &self.base.gpu_a, &self.gpu_p, 0.0, &self.gpu_v);
            ctx.queue.submit([enc.finish()]);

            let r0_dot_v = self.base.dot_readback(ctx, &self.gpu_r0, &self.gpu_v);
            if r0_dot_v.abs() < 1e-30 { break; }
            alpha = rho / r0_dot_v;

            // s = r − α·v   (store in gpu_s)
            let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("bicgstab_s") });
            self.base.vops.encode_axpy(ctx, &mut enc, -alpha, &self.gpu_v, 1.0, &self.base.gpu_r);
            self.base.vops.encode_axpy(ctx, &mut enc, 1.0, &self.base.gpu_r, 0.0, &self.gpu_s);
            ctx.queue.submit([enc.finish()]);

            // t = A·s
            let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("bicgstab_t") });
            self.base.spmv.encode_spmv(ctx, &mut enc, 1.0, &self.base.gpu_a, &self.gpu_s, 0.0, &self.gpu_t);
            ctx.queue.submit([enc.finish()]);

            let t_dot_t = self.base.dot_readback(ctx, &self.gpu_t, &self.gpu_t);
            let t_dot_s = self.base.dot_readback(ctx, &self.gpu_t, &self.gpu_s);
            omega = if t_dot_t.abs() > 1e-30 { t_dot_s / t_dot_t } else { 0.0 };

            // x = x + α·p + ω·s
            let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("bicgstab_x") });
            self.base.vops.encode_axpy(ctx, &mut enc, alpha, &self.gpu_p, 1.0, &self.base.gpu_x);
            self.base.vops.encode_axpy(ctx, &mut enc, omega, &self.gpu_s, 1.0, &self.base.gpu_x);
            ctx.queue.submit([enc.finish()]);

            // r = s − ω·t
            let mut enc = ctx.device.create_command_encoder(&wgpu::CommandEncoderDescriptor { label: Some("bicgstab_r") });
            self.base.vops.encode_axpy(ctx, &mut enc, -omega, &self.gpu_t, 1.0, &self.gpu_s);
            self.base.vops.encode_axpy(ctx, &mut enc, 1.0, &self.gpu_s, 0.0, &self.base.gpu_r);
            ctx.queue.submit([enc.finish()]);
        ...
        _x.copy_from_slice(&self.base.read_solution(ctx));
        Ok(SolveResult { converged: r_norm <= tol, iterations: iters as usize, final_residual: r_norm })
    }
}

pub fn solve_bicgstab_gpu(ctx: &GpuContext, a: &CsrMatrix<f64>, b: &[f64], x: &mut [f64], cfg: &SolverConfig) -> Result<SolveResult, SolverError> {
    let mut ws = BicgstabGpuWorkspace::<f64>::new(ctx, a, b);
    ws.solve(ctx, x, cfg)
}

pub fn solve_bicgstab_gpu_f32(ctx: &GpuContext, a: &CsrMatrix<f32>, b: &[f32], x: &mut [f32], cfg: &SolverConfig) -> Result<SolveResult, SolverError> {
