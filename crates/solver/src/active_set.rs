...
pub fn solve_active_set_contact<M: MeshTopology>(
    ...
    for f in 0..mesh.n_boundary_faces() as u32 {
        if !cs.contains(&mesh.face_tag(f)) {
            continue;
        }
    ...
    for f in 0..mesh.n_boundary_faces() as u32 {
        ...
        let p = [
            mesh.node_coords(fn_[0]),
            mesh.node_coords(fn_[1]),
            mesh.node_coords(fn_[2]),
        ];
        ...
        for &ni in fn_ {
            node_normal[ni as usize] = [nx / al, ny / al, nz / al];
        }
    }
    ...
    for iter in 0..max_iter {
        ...
        for ni in 0..n_nodes {
            ...
            for d in 0..dim {
                let di = dofs[d];
                let s = a_mod.row_ptr[di];
                let e = a_mod.row_ptr[di + 1];
                for p in s..e {
                    if a_mod.col_idx[p] as usize == di {
                        a_mod.values[p] += pen * nu[d] * nu[d];
                    }
                }
