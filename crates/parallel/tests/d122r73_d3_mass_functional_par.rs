...
fn d122r73_cross_rank_matrix_entries_agree() {
    ...
    for n_ranks in [2usize, 4] {
        let mesh = mesh.clone();
        let maps: Arc<Mutex<Vec<(usize, HashMap<(u32, u32), f64>)>>> =
        ...
        ThreadLauncher::new(WorkerConfig::new(n_ranks)).launch(move |comm| {
            ...
            maps_rank.lock().unwrap().push((rank, m));
        });
        let maps = maps.lock().unwrap().clone();
        let (mut diff, mut common) = (Vec::new(), 0usize);
        for (i, (_, mi)) in maps.iter().enumerate() {
            for (_, mj) in maps.iter().skip(i + 1) {
                for (k, v0) in mi {
                    if let Some(v1) = mj.get(k) {
                        common += 1;
                        if (v0 - v1).abs() > 1e-12 * v0.abs().max(v1.abs()).max(1e-30) {
                            diff.push(format!(
