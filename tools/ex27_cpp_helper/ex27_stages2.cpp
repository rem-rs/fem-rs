...
Mesh *Build(int ref, int stage)
{
   ...
   if (stage >= 1) { mesh->SetCurvature(3, true); }
   if (stage >= 2) { for (int l = 0; l < ref; l++) { mesh->UniformRefinement(); } }
   if (stage >= 3) { mesh->Transform(trans); }
   return mesh;
}
int stage_main()
{
   ...
   {
      Mesh *m = Build(2, s);
      std::cout << "stage " << s << ": nv=" << m->GetNV();
      std::cout << "  v0=(" << m->GetVertex(0)[0] << "," << m->GetVertex(0)[1] << ")";
      if (m->GetNodes())
      {
         GridFunction &nd = *m->GetNodes();
         int n = nd.Size();
         // find a node with x=-1 and one with x=0
         double xm = 999.0, x0 = 999.0;
         for (int i = 0; i < n; i += 2)
         {
            double x = nd(i);
            if (x < -0.99) { xm = x; }
         ...
         std::cout << "  node x=-1 present: " << (xm < -0.99) << "  x=0 present: " << (x0 == 0.0);
      }
      std::cout << "\n";
      delete m;
   }
