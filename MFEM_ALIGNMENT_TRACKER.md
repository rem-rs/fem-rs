...
- ✅ **HDF5/XDMF imported-mesh result workflow regression**（2026-05-15）。`crates/io/tests/io_integration.rs` 新增 `hdf5_xdmf_imported_mesh_preserves_tags_and_result_metadata_workflow`，端到端验证导入网格写入 HDF5 后 `elem_tags`/`face_tags` 保持不变、节点结果场可以从 HDF5 回读、并且 XDMF sidecar 同时暴露 node-centered 结果字段和 cell-centered `material_id` 数据集；已接入 `.github/workflows/alignment-smoke.yml` 的 `io-mesh-coords-checkpoint` suite。
- ✅ **Abaqus named-set user-closure regression**（2026-05-15）。`crates/io/tests/io_integration.rs` 新增 `abaqus_named_sets_and_result_export_workflow`，端到端验证 `read_abaqus_inp_full()` 提供的 `elset/nset` 名字可直接转成用户配置掩码字段，并通过 HDF5/XDMF sidecar 同时导出 node-centered `fixed_mask` 与 cell-centered `material_id`；已接入 `.github/workflows/alignment-smoke.yml` 的 `named-attrs-io` suite。
- ✅ **Netgen surfaceelements user-closure regression**（2026-05-15）。`crates/io/tests/io_integration.rs` 新增 `netgen_surfaceelements_boundary_mask_and_result_export_workflow`，端到端验证显式 `surfaceelements` 边界标签在 `.vol` 导入后可直接生成用户边界掩码字段，并通过 HDF5/XDMF sidecar 同时导出 node-centered `boundary_mask` 与 cell-centered `material_id`；已接入 `.github/workflows/alignment-smoke.yml` 的 `io-mesh-coords-checkpoint` suite。
- ✅ **Example-level named-attribute VTK workflow**（2026-05-15）。`examples/mfem_ex39_named_attributes.rs` 新增 `--vtk <path>` 用户出口，可将 named sets 直接导出为 `inlet_mask`、`outlet_mask`、可选 `merged_boundary_mask` 以及 cell-centered `fluid_id` VTK 字段；测试 `named_attributes_vtk_export_writes_boundary_and_material_fields` 已接入 `.github/workflows/alignment-smoke.yml` 的 `named-attrs-io` suite。
- ✅ **Example-level named-set solve workflow**（2026-05-15）。`examples/mfem_ex39_named_attributes.rs` 新增 `--solve-poisson` 模式，在 Gmsh named sets 基础上直接施加 `inlet=1/outlet=0` 的用户边界配置并求解双材料扩散问题：材料系数通过 `PWConstCoeff` 按 named region 对应的 `elem_tags` 注入，区域源项则通过 `extract_submesh_by_name() -> assemble_linear() -> transfer_to_parent()` 的链路按 named region 回填到父网格；VTK 输出现同时包含 `u`、`inlet_mask`、`outlet_mask`、可选 `merged_boundary_mask`、`material_id`、`kappa`、`source_strength`、`fluid_id` 和 `liner_id`。测试 `named_attributes_poisson_solve_respects_named_boundary_values` 与 CLI smoke 已接入 `.github/workflows/alignment-smoke.yml` 的 `named-attrs-io` suite。
- ✅ **Example-level imported config workflows**（2026-05-15）。`examples/mfem_ex39_named_attributes.rs` 现已提供 `--abaqus-demo/--abaqus` 和 `--netgen-demo/--netgen` 入口：Abaqus 路径可将导入的 named node/element sets 直接转为求解约束与 `u/fixed_mask/drive_mask/material_id` VTK 字段，Netgen 路径可将导入的边界 tag 直接转为 `boundary_mask/material_id` VTK 字段。另已新增文件级 example fixture `[examples/meshes/named_sets_tet.inp]` 与 `[examples/meshes/surface_tags_tet.vol]`，以及文档 `docs/superpowers/specs/2026-05-15-imported-named-config-workflows.md`；测试 `abaqus_named_sets_drive_example_solution_and_vtk_fields`、`netgen_boundary_tag_example_exports_mask_field`、`abaqus_file_fixture_workflow_exports_fields`、`netgen_file_fixture_workflow_exports_fields` 和对应 CLI smoke 已接入 `.github/workflows/alignment-smoke.yml` 的 `named-attrs-io` suite。
