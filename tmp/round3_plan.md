...
- **D98（P2，仍未做）NURBS 跨空间混合积分器**（`nurbs_ex24` 三档）：recipe 见第二十七轮债务（`MixedVectorIntegrator::AssembleElementMatrix2`：`w = Trans.Weight()·ip.weight`、求积阶 `trial+test+Trans.OrderW()`、Gradient 的 trial = `CalcPhysDShape`、Curl 的 trial = `CalcPhysCurlShape`）。`-p 2` 只缺 PCG+DSmoother（D97 已备件）。
- **D112（P2）`Cubic`（MFEM 遗留 FE 集合名）网格的 p=3 dof 布局读错**：`fichera-q3` 等 **9 个**网格的单元中心 min det 为负（MFEM `Weight()` = +1.33797 vs fem-rs −0.0401），而显式 `H1_3D_P3` 正常 ⇒ 嫌疑 `crates/io` 的 `nodes` 读取或 `HexQk::new(3)` slot 序。**建议单独立项**（可能是又一个真内核 bug）。
- **D113（P2）`refine_uniform_3d` 静默丢弃高阶几何**：只在"恰好 order 2 + 27 dof/hex"时搬运几何（`cube.mesh -o 3 -rs 1` 细化后 geom_order 3→1；Hex20/Hex27 分支内部 `geometry: None`；tet/prism/pyramid 仅线性）⇒ 需把 `curved_hex.rs` 从 Q2 泛化到任意阶。
- **D114（P3）`refine_mixed_3d` 在 `data/fichera-mixed-16.mesh` 上 panic**（`amr_inner.rs:1525`，tet MarkEdge 缺边）。
