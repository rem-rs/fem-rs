...
| **P1（部分关闭，round 76）** | **D807-1 部分**（`par_partition` 发布全网 share-set 最小 owner + facet 最小全局单元锚（新 `partition.rs::EntityOwnership`，随 `MeshPartition` 传递；非提取分区为 `None` ⇒ 串行/AMR 逐位不变）；层 `cylinder-hex` np2 **252 → 222/246**、np4 **252 → 222/222/246/234**；**未达** 180/204 的单层节点闭包，残余 = facet 的 position/sign 仍从**锚单元**读出 ⇒ 锚单元必须本地。**下一步：把 position/sign 本身发布出去**。注：MFEM 的 `GetNE()`/`GetNV()` 只数 owned（ghost 在 `face_nbr_elements`/`face_nbr_vertices`）⇒ **不要拿 126 当目标**）；**D807-2**（tet NDk≥2 逐面点对变换通道；**本轮未动**，`crates/space` 未改） | **round 76 部分 / D807-2 待派** |
| **P1（新，round 76 登记）** | **D812-1**（**写者按 `geom_order() > 1` 才写 `nodes`** ⇒ order-1 几何场回环**静默丢失**；`data/periodic-{hexagon,segment,square,cube}.mesh` 四份自带 `L2_T1_2D_P1`；实测 `read`：`geometry_table=true`、写回 `has_nodes_section=false`；`ex9.mesh` 411 行/8428 B vs C++ 993 行/16862 B；**oracle 已捕获** `/home/quan/work/d76main/ex9cpp/ex9.mesh`；改发射条件会动所有 `.mesh` 写路径 ⇒ 必须自带字节金标回归）、**D812-2**（ex9 `.gf` 两侧尺寸 4× 差（9836 vs 39891 B）而步进轨迹逐行同 ⇒ 输出格式问题，疑与 D812-1 同源）、**D808-4 残余**（`build_tet4_pa_data` 是**单质心 QP**，曲面网格上结构性不等价 ⇒ 需多 QP 数据布局）、**D810-1 收窄**（`wg` 族弦长路线含面**测度**、面**物理点**、**参考点**反解**三样** ⇒ 修法是**整条面路径**交 `dg_base::face_point_geom`/`_3d`；**该族仓内无任何真值 oracle ⇒ 先造 oracle 再改**） | 待派 |
