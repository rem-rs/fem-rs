...
| **P1（新，round 69 登记）** | **D729**（hex 外 [-1,1] 表 shim 清单：pa/prism_pk 轴向、geometric_mg SumFact ~8.6e-6、pyramid D339）；**D730**（solver CG/SpMV/ARF op-order）；**D731**（legacy [-1,1] 帧处置）；**D732**（geometry_jacobian 曲面 hex 仍直化，消费方 mixed_hyperelasticity）；**D733**（1-D ND_R1D 未移植，C++ 金标 ARF 0.124887/‖E−E‖ 0.226983 已留档）；**D734**（曲面 quad/tet/tri/prism dyn 路径不对称）；**D735**（multidomain IC 投影尘，crates 层）；**D736**（全默认档 stdout HYPOTHESIS 待长窗口）；**D737**（multidomain.rs H1 变体 canonical 面）；**D738**（ex22 2-D ND 评估器 vs ComputeL2Error——D719 根因，管线已证=MFEM，oracle 已备）；**D739**（D706 串行入口消费方迁移积压）；**D740**（ND1-quad IMAG 幽灵存储 ~1e-16，零求解影响）；**D741**（ex22 -p2 双侧预条件子升级可选） | 待派 |
