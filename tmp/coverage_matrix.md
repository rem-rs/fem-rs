...
| **P1（新，round 67 登记）** | **D708（round 68 头号）**`fem_space::hdiv` RT1-hex 内部 dof 槽位/符号 ↔ HexRTk 基在曲面 hex 上的对应（multidomain cyl 最后一公里：sum −1.42e-3 vs −6.4e-6；多重集/不变量类检查对此盲——D709 建议升级 P-共轭性 pin；证据链 tmp/d690/）；**D702**（ex22 -p0 3-D 解层偏差：hex o1 0.2128 vs 0.1459）；**D712**（ex24 -p1 iter1/ARF 位级：复刻 CalcVShape/CalcCurlShape 累加序，splice 基建已备）；**D713**（D690 按 D680 后新轨迹重验收——主会话已跑合流态：block 6-7 位、cyl ssq +17%，余 D708 主导）；**D706**（FormLinearSystem 形状核心入口——D697 事故面根治）；**D714**（跨 libm sin 1 ulp 平台事实：glibc vs UCRT，Linux 参考 vs Windows Rust 位级门的约束，非 bug）；**D703 余项**（pex3 -o2 误差行[随 D697 已部分改善] + 标量 AMG vs HypreAMS 家族差）；**D696 批 3/4**（postproc + iga/wg/reed/ad/dg/physics） | 待派 |
