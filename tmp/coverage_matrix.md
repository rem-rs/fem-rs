...
| **P1（新，round 71 登记）** | **D771**（ex27 canonical stdout 格式——Options 块/"Solved in" 多行/%g6，照 ex1/ex24 定式；数值已全对）；**D765**（2-D quad NDk k≥3 空间/装配器配对不一致——两候选元实测各失败、`interpolate_vector` 缺 quad k≥3 支，待 MFEM `ND_QuadrilateralElement(3)` 逐槽裁决）；**D766**（`fem_linalg::Vector::dot` rayon sum = D754 根因，修法=D746 同款）；**D762**（rt IC 尘行计数 512→448 漂移，worktree 二分）；**D763**（**`-vs` 参与求解轨迹**——三档对比/回归必须钉死 `-vs`）；**D764**（round-64/69 旧 print-ref 夹具 SUPERSEDED 标记）；**D767**（HDiv 求解不收敛：2-D tri RT1/3-D tet RT1-2，评估器无关，预处理器侧）；**D768**（2-D serendipity `QuadSerendipityPk` 仍 [-1,1]²+`quad_rule`，潜伏）；**D769**（SumFact `mult_raw/mult_constrained` p≥5 scratch 越界 panic）；**D770**（prism PA 用 FD Jacobian + 只取首 QP 的 |detJ|/κ，潜伏）；**D752**（值列表消元入口族收敛）/D754（→D766）/D755（D706 覆盖缺口）/D756（pex34 ranks2）/D758（ex34 -hex/ex29 停判据/LOR 门限）/D759（mms ND1 帧残留）/D750/D751/D729 余项（pyramid D339）维持 | 待派 |
