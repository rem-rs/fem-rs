...
| 并行 | BIT（np1/2/4 键表逐位） | D124/D136/D156/D504；**round 72：D122 登记症状 = 过期行**（隔离 worktree `af9c765c` 红证据：cylinder-hex ranks 2/3/4 = 336/816/1172 条 sentinel、64 元 hex = 64 条 + `ghost.rs:178` panic；HEAD 全扫 6 网格 × ranks{1..4} × 9 族 = 216 档全绿 ⇒ 已由 D412[round 48] 关闭）⇒ **D122-1/-2/-3 已在 round 73 处置**（D122-1：owner 改"持有该实体的单元 min owner" ⇒ **九空间 per-rank owned 逐位 = MFEM `GetTrueVSize()`**（np1/2/4）、np1 逐字节零变化；D122-2：**前提被驳**——照字面做单层 np=4 直接 panic（持有者跨 rank 不可答）⇒ fixpoint 是**最小可行集**，重登记 D807-1；D122-3：**真因 = 面 dof 符号修正缺失**（非登记的"面 dof 键"），修后 `disagreeing` 24012 → 0、守恒 ND2 **4.03e-14**/ND3 **1.42e-13**）。**round 76 推进**：`par_partition` 发布 `EntityOwnership`（全网 share-set 最小 owner + 每个 facet 的最小全局单元锚），层 `cylinder-hex` **252 → np2 222/246、np4 222/222/246/234**；**残余 = D807-1**（facet 的 position/sign 仍从**锚单元**读出 ⇒ 锚单元必须本地，未达 180/204 单层闭包；MFEM 的 `GetNE()`/`GetNV()` 只数 owned，ghost 在 `face_nbr_elements`/`face_nbr_vertices` ⇒ 不要拿 126 当目标）与 **D807-2**（tet NDk≥2 逐面点对变换通道，**未做**）。锚点：pex3 四档逐位（87/1/95/193 it）、九空间 owned 逐位、串行/AMR 路径逐位不变。残：D527 连续分块（故意不动） |
...
| **P1（新，round 72 登记）** | **D772**（`mixed::ref_elem_vec` HCurl **tri** 臂缺 o≥2：o=2 拿 `TriNDk(2)` 配 `TriND2` 空间表）；**D773**（`dpg_basis::hcurl_ref_elem(Tri3,p)` p=2 同族潜伏）；**D774**（`pa::prism_pk` 全量跑偶发 flake）；**D775**（`lor_factory.rs:564/1337` 注释"库默认 quad 元 = legacy QuadNDk"已过期）；**D776**（`vector_assembler.rs` 的 `n_i>0 && Quad4` bubble 分支 `[−1,1]²` 语义，当前不可达）；**D777**（**D721 残项：GPU hex WGSL 着色器**——`bary/dary` 节点已在 round 72 重生成（两 pin 修前红），但 `GP/GW` 求积常数仍在 `[-1,1]` 而全树（element `hex_rule`/CPU PA `gauss_legendre_01`）是 `[0,1]` ⇒ 需 generator 改发 `[0,1]` 规则 + q1..q4 重生成 + 求积帧 pin + GPU 数值 pin；分析在 `tmp/d721x/README.md`）；**D778**（ex27 边界平均残差 = `integrate_bc` 半面求积[`[0,1]` 点喂 `[-1,1]` 映射] + 网格构造顺序差；加性账目已闭合，配方在案，pex27 同款）；**D779**（ex27 `-dg` 未对齐：第 0 步 `(B r,r)` 0.142775 vs 0.0220206 + `rtol` 语义）；**D780**（ex22 pc 元素级合成：`‖M b‖` 首值仍 2.20×、收敛档普遍 +1 迭代）；**D781**（`vector_assembler` 矢量线性形并行归约未审计——主会话筛为阴性[ex31 3 连跑+线程扫全同]，配方在案）；**D782**（MFEM 2-D serendipity `H1Ser_*` 语义端口缺失：GLL/`S_p`/`(p²+3p+6)/2` dof）；**D783**（曲面棱柱 PA 侧仍用 6 顶点三线性几何）；**D784**（prism PA 测试均单元素，无多元素共享 dof 的 PA-vs-装配 pin）；**D785**（joule.rs:663 的 `n_bdr` 取自本地网格 ⇒ np≥2 ess 掩码与 C++ 不一致）；**D786**（np > 单元数时空 rank 在 `HCurlSpace::new`[hcurl.rs:852] panic + ThreadLauncher 挂住）；**D787（重要）**：`fem_mesh::transformation::element_jacobian_at` 对**曲面 tet/prism 无等参分支**（走直线映射；实测母单元体积 2.0e-1 vs 真值 1.8298e-1 = **9.3% 偏差**），而 `compute_l2_error`/`hdiv_error`/`flux_recovery`/`dgmassinv`/`complex`/`topology_optimization` 均消费它 ⇒ 曲面单纯形网格上的误差估计器/后处理受影响（配方 `tmp/d113/README.md §4.2`）；**D788**（D113-2：Pyramid5+曲率搬运；**MFEM 4.10 自己细化曲面金字塔必 SIGSEGV**[ASan 栈 `UpdateNodes→RefinementOperator::Mult→Table::GetRow`] ⇒ 真值不可得，HYPOTHESIS 级）；**D789**（D113-3：serendipity 三族 Hex20/Prism15/Pyramid13——读者有意不附几何表 + MFEM 拒绝 gmsh 码 16-19 ⇒ 无真值；配方 = `findpts::incomplete::eval_map` 做父求值器）；D752/D755/D756/D758/D759/D750/D751/D729 余项（pyramid D339）维持 | 待派 |
| **P1（round 73 关闭账）** | ~~D122-1~~（owner 改"持有该实体的单元 min owner"；**九空间 per-rank owned 逐位 = MFEM `GetTrueVSize()`**（np1/2/4）+ np1 逐字节零变化）、~~D122-3~~（**根因 ≠ 登记的"面 dof 键"**：`from_edge_space_ordered` 只给**边** dof 写符号修正、面 dof 恒 1.0，而 DP 全局基与局部子网格锚差一个**带符号置换**（修前 `common=642850/disagreeing=24012` 全为 `v1 == −v0`，修后 0）；守恒 ND2 4.03e-14/ND3 1.42e-13）、~~D790-1~~（本轮引入的 AMG 停滞回归：ghost-aware 粗化，pex3 np2 **10000 it → 95 it**，np1 锚逐位不动）、~~D780~~（**真因反转**=GMRES 初始猜测 `X` 而非 pc 合成；tri `-p2 -o1` 首残 0.255409= C++、266 it= C++，"+1 迭代"族全消）、~~D778~~（ex27 半面求积 ⇒ **default 51/51 + dbc25 52/52 逐字节**）、~~D787~~（曲面单纯形等参 Jacobian：tet P2 +9.30% → 2.5e-16；消费方 `_l2` 5.26% → 1.7e-16）、~~D793-1/-2/-3~~（flux_recovery 单纯形臂；**3-D 里的 2×2 行列式真 bug**；`needs_iso` 名单缺族 + 顺带修 Hex27/Pyramid13/Prism18 的直线档 L2 范数）、~~D777~~（GPU hex WGSL 三缺陷：求积帧 + **生成文本缺 `cs_main` 收尾 `}` ⇒ 根本无法编译**（自 `2af8c124` 起即如此）+ q1/q2 未随 D721 迁移；四个 `.wgsl` 重生成 + 帧/语法 pin + 真设备 f32 数值 pin）、~~D772/D773~~（**裁决：`TriND2` 与 `TriNDk::new(p)` 是同一个元**（基差 3.6e-15）⇒ 是缺臂 + 命名不一致，两处都改走 `paired_vector_reference_element`）、~~D795-1~~（**前提被驳**：MFEM 面规则在 `[0,1]`/`Σw=1`、直边 `|nor|=h`；dg.rs 的"经验减半/×4"都在补错前提 ⇒ 改等参几何，256 条目 1.370e0 → 1.492e-13；ex14 DG Poisson 迭代史逐字节 = C++）、~~D775/D776~~（过期注释；两份重复气泡基收敛成一处 [0,1]² 实现）、~~klein flake~~（同文件两测试共用 `CARGO_TARGET_TMPDIR/klein-bottle.mesh` ⇒ 并发门间歇红，已修） | — |
| **P1（新，round 73 登记）** | **D790-1a**（`use_global_aggregation` 现为文档化 no-op，7 个示例仍设置该字段 ⇒ 删字段 + 改示例调用点）、**D790-2**（D122-1 的跨 rank 论证依赖全 ghost 层 = D122-2）、**D790-3**（tet k≥2 面 dof 的 element↔canonical 是 2×2 旋转，标量符号表达不了；本轮夹具 hex-only）、**D793-4**（`use_iso` 名单一致性——**不能**按原登记补 Prism18/Pyramid13，会 panic）、**D793-5**（MFEM `GetElementVolume` 自身在曲面元上欠积分，事实记录）、**D795-1 余项→D799-1**（**ex27 `-dg` 残余 = 网格表示不同**：C++ 折回 302 顶点/96 bdr/tags 1-4 vs Rust 311/112/tags 1-6 ⇒ DG 无 dof 识别、缝上零耦合；配方 = `InteriorFace` 支持两侧节点对或示例重建折回网格）、**D799-2/D799-3**（ex27 DG RHS helper 硬编码 Quad4；其余 DG 示例面几何未切换）、**D797-1（重要）**（`mixed::HCurlH1WeakDiv` 对物理梯度又乘 `J⁻ᵀ/detJ` ⇒ 单元矩阵按 c⁻³ 缩放而 MFEM 形式尺度不变；消费方 DivergenceFreeProjector/par_mixed_assembler ⇒ 独立改+重验）、**D797-2/D797-3**（`quad_q1.wgsl` [-1,1] 自洽但无帧 pin；hdg/wg 用 `TriNDk::new(2)` 命名）、**D800-1**（`use_iso` 臂应传 `geometry_nodes` 而非 `element_nodes`——D793-4 的正确配方）、**D800-2**（`flux_recovery` 对 embedded 曲面单纯形无 3×2 契约，panic）、**D795-2/-3/-4/-5**（ex22 非 inline BC 投影；pex27 DG 桩序；ex27 DG helper 物理点反解 + 仅 order 1；网格构造序余项） | 待派 |
| **P1（round 76 关闭账）** | ~~D805-1~~（**DG 弹性罚项 = MFEM 原式**：`jmatcoef = kappa·(nor·nor)·wLM`，内部面 `wLM=(ipw/2)·((λ₁+2μ₁)/W₁+(λ₂+2μ₂)/W₂)`、边界面 `ipw(λ+2μ)/W`，用**未归一化** `nor = CalcOrtho(Trans.Jacobian())` 与 `Weight()`=带符号 det J；旧码取 λ,μ 平均又除以 `|nor|`；顺带新增 `mfem_elasticity_volume_rule`（体项默认阶 `trial+test+OrderW`，**公开签名不变、直网格不动**）；`MATELA` 4.149e-1 384/1024 → **7.501e-12 2/1024**）、~~D805-2~~（**DG 对流面项换逐行移植的 `DGTraceIntegrator`/`NonconservativeDGTraceIntegrator`**（新 `crates/assembly/src/dg/dg_trace.rs`；`dgtrace_weights` 是内部面/边界面/周期缝**共用唯一算术源**；`DGAdvectionIntegrator` 加 `alpha` 字段 = ex9/ex41 的 −1.0）；旧实现是守恒型迎风 `−∫⟦v⟧F̂`、与任何 MFEM 积分器都不逐条目等价而注释声称等价；`MATADV` 1.000e0 48/256 → **1.388e-14 0/256**；主会话另用 MFEM 自身输出证 `MATADV == −transpose(MATADV2)` 精确 0.000e0）、~~D808-4~~（**hex/quad PA 核在 `geom_order ≥ 2` 走 order-g 等参几何**（新 `pa/curved.rs::curved_jacobian`，`geom_order ≤ 1` 逐字回旧路径）；14 个核配置 **7.9e-16…3.3e-15** vs 修前 **3.4e-2…1.0e-1**（齿在测试内）；`quad_q1` 的 `[-1,1]²` 帧须把 Jacobian 减半；**登记漏了 `hex_q1`，一并修**；`tet4` 单 QP 质心结构性不等价 ⇒ 见新债）、~~D805-3~~（**ex27 `refined.mesh`**：真因 = 周期缝合保留 pre-merge 几何快照 ⇒ 需**不连续** `nodes`（MFEM `SetCurvature(3,true)`）；新增 `write_mfem_file_nodes` + `NodesSpace::Discontinuous`；MFEM 读回 **TOPOLOGY-IDENTICAL**、`fec=L2_T1_2D_P3`、nodes 4.29e-08；ex27 **10/10 逐字节**）、~~D805-5~~（`face_point_geom` 法向定向，round 75 随 D799-3 修） | **round 76 关闭** |
| **P1（部分关闭，round 76）** | **D807-1 部分**（`par_partition` 发布全网 share-set 最小 owner + facet 最小全局单元锚（新 `partition.rs::EntityOwnership`，随 `MeshPartition` 传递；非提取分区为 `None` ⇒ 串行/AMR 逐位不变）；层 `cylinder-hex` np2 **252 → 222/246**、np4 **252 → 222/222/246/234**；**未达** 180/204 的单层节点闭包，残余 = facet 的 position/sign 仍从**锚单元**读出 ⇒ 锚单元必须本地。**下一步：把 position/sign 本身发布出去**。注：MFEM 的 `GetNE()`/`GetNV()` 只数 owned（ghost 在 `face_nbr_elements`/`face_nbr_vertices`）⇒ **不要拿 126 当目标**）；**D807-2**（tet NDk≥2 逐面点对变换通道；**本轮未动**，`crates/space` 未改） | **round 77：D807-2 关闭（见下）；D807-1 残余重登记 = D813-1** |
| **P1（新，round 76 登记）** | **D812-1**（**写者按 `geom_order() > 1` 才写 `nodes`** ⇒ order-1 几何场回环**静默丢失**；`data/periodic-{hexagon,segment,square,cube}.mesh` 四份自带 `L2_T1_2D_P1`；实测 `read`：`geometry_table=true`、写回 `has_nodes_section=false`；`ex9.mesh` 411 行/8428 B vs C++ 993 行/16862 B；**oracle 已捕获** `/home/quan/work/d76main/ex9cpp/ex9.mesh`；改发射条件会动所有 `.mesh` 写路径 ⇒ 必须自带字节金标回归）、**D812-2**（ex9 `.gf` 两侧尺寸 4× 差（9836 vs 39891 B）而步进轨迹逐行同 ⇒ 输出格式问题，疑与 D812-1 同源）、**D808-4 残余**（`build_tet4_pa_data` 是**单质心 QP**，曲面网格上结构性不等价 ⇒ 需多 QP 数据布局）、**D810-1 收窄**（`wg` 族弦长路线含面**测度**、面**物理点**、**参考点**反解**三样** ⇒ 修法是**整条面路径**交 `dg_base::face_point_geom`/`_3d`；**该族仓内无任何真值 oracle ⇒ 先造 oracle 再改**） | **round 77：D812-1/D812-2 关闭（见下表）；D808-4 残余、D810-1 维持开放** |
| **P1（round 77 关闭账）** | ~~D812-1~~（**`.mesh` 写者 `nodes` 发射跟随"几何表存在"**（MFEM `Nodes != NULL`；唯一丢弃点是 `SetCurvature(order≤0)`，`mesh/mesh.cpp:7214` vs `:12551`）：旧门是 `geom_order() > 1` ⇒ order-1 表按直网格写。**premise 更正**：四份 `periodic-*.mesh` 的**折叠**表对**连续**写者是**响亮拒绝**（D805-3 的连续性检查），**静默丢失的是 Discontinuous 臂**（旧 `order ≤ 1 ⇒ Ok(None)` 先于两族分支）；`make_periodic` 网格在两侧都**是** L2 表（MFEM `Mesh::MakePeriodic` 自身 `SetCurvature(nodal_order, discont=true)` 于重编号前，`mesh.cpp:6205`）；**oracle**：`write_mfem_nodes(.., Discontinuous)` 与 MFEM 自己的 `Mesh::Save(out,16)` 重存**逐字节相同**（hexagon 1402/1402、square 1109/1109、cube 6392/6392 B），连续臂亦 = `SetCurvature(1,false)` 重存，Tri3/Prism6 order-1 同；**爆炸半径经枚举 = ex9 + ex18**（其余 `periodic-*` 消费者不写网格）⇒ ex18 由 `write_mfem_file` 改 `write_mfem_file_nodes(.., Discontinuous)`（C++ 对照 213 行两侧、结构逐 token 同、worst 9.0e-09）；**全量金标账** `crates/io/tests/d812r77_mesh_write_ledger.rs` 钉 106 份 `data/*.mesh` 的 status/digest/bytes/lines/geometry/nodes/拒绝原因，无几何表者必须逐字节不变（diff 唯一行为 hunk 就是门）；Tet4 order-1 编号仍拒（仓内无已验证的 MFEM L2 tet 编号））、~~D812-2~~（**"4× 尺寸差"不存在**：pre-fix 序列 ex9 在 mesh 写出处 **abort**（rc=101）⇒ 注册的 411 行/8428 B、9836 vs 39891 B 不对应任何一次运行；实测 mesh 993 行两侧、`.gf` 3077 行两侧、尺寸差 4%。**两处真 `crates/io` 偏差已修**：① `.gf` 值在 `precision < 16` 曾走 `{:.prec$e}`（永远科学计数）而 MFEM `Vector::Print(os,1)` 用流 defaultfloat `%g`（oracle 20 值中 11 个是定点）⇒ 改 crate 内 `format_g`，同时删死代码 `c_printf_g16`；② 示例传 FEC 名 `L2`（渲染 `L2_2D_P3`）而 MFEM 写 `L2_T1_2D_P3`（空间确是 GLL）+ 精度 7 → 8。**残差 = 包围盒不是初值**：`ex9.cpp` 的 `GetBoundingBox(bb, max(order,1))` 在 `Nodes != NULL` 时按 `GlobGeometryRefiner` 采样几何取极值，而示例用读入器的 pre-refinement **顶点**极值；两侧都**是插值**（MFEM `ProjectCoefficient` 默认路径 = 节点求值 `fem/gridfunc.cpp:2450`，且 `postproc::project_coefficient` **就是** `space.interpolate`）⇒ 原"1:1 改投影"是 no-op。ex9 stdout 现与 C++ oracle **逐字节相同**（仅 `--mesh` 路径回声 + stderr wall-clock 两处具名豁免））、~~D807-2~~（**tet NDk≥2 面 dof 对得 2×2 通道**：space 侧 `FacePairTransform` + `HCurlSpace::element_face_pair_transforms`（hex/2-D/k=1 **为空** ⇒ 那些路径不可能动）+ `FESpace` 默认空体；DP 侧 Pass B 记**锚单元**的非恒等块 + `dual/primal_dof_transform`，`permute_csr/_scaled/_vec` 全走它（标量是同一代码的 `(d,±1)` 特例，逐位不变），`par_l2zz{,_3d}` 在标量逆错时**响亮拒绝**；组合 = `A_global = s⁻ᵀ·A_canon·s⁻¹`，**两侧同一系数表**（转置列侧 = 3.4e-2 残差的真因，标量通道因 ±1 自逆而看不见）；**k=2 np2 rel 6.7e-1 → 3.6e-16，k=3 1.087 → 5.8e-16；跨 rank 全局条目 0/736、0/3825**（标量控制 63、441，红对照在测试内）；np1 无通道（断言）；**更正**：非对称夹具锚块 = `[[-1,0],[-1,1]]`（既非旋转也非切向交换，仍非标量） | **round 77 关闭**；**收尾另闭**：D805-3 的兄弟（ex27 `sol.gf` 与 C++ 尺寸不同）= **配置混淆**（`-rs 2` 两侧 3089 B/`diff` 0 行、`-dbc 2.5 -rs 2` 两侧 2870 B/`diff` 0 行；**金标文件名不是配置权威，自带的 Options 头才是**） |
| **P1（round 78 关闭账）** | ~~D813-1~~（**D807-1 残余关闭：ghost 层砍到单层节点闭包**——提取侧发布 facet 规范锚的`(ElementType, 全局顶点表)`（`EntityOwnership::facet_anchor_element`；**submesh 线格式新 flag bit 32**，复用既有 ElementType tag 对 + `n_verts==nodes_per_element` 校验 + wire 往返测试）；**删除 step 3b2(a)**；面规范基由空间从 facet 自身重建（`HCurlSpace::facet_slots_against_published_anchor` + `OFF_FACET_SENTINEL` + 双射守卫 ⇒ 非面局部族**拒绝**；`FESpace` 默认 `None` ⇒ pro 层零改动）；DP 的 NDk Pass B 消费通道（pair 按锚分量序、第二载体分歧即响亮 assert），**HDiv/RT 的 `from_face_space` 同样改接发布锚**（经 `hdiv::{quad_orientation,tri_orientation,transform_grid,tri_face_grid_transform}`）。**实测层**：np2 **180 (126+54) / 204 (126+78)** 元、268/347 点；np4 **126/144/166/126**、220/247/313/248 点（修前 222/246、222/222/246/234）；np1 与 `entities=None` 逐位不动；九空间 owned 逐位 = `GetTrueVSize`；途中两通道 bug：creator face-key 的**局部 vs 全局 id 域错配**（曾 100% 拒绝）与 tri 族 **per-point vs per-slot** 数量错配（全 hex 网格不可见、tet/prism oracle 抓住）；RT 曾单独红 4.6e-4…9.5e-3（`from_face_space` 未接））。~~D813-2~~（**登记前提被驳：读入器顶点表规则是"对每个引用单元取算术平均"**，非 last-wins——round-77 的 last-wins oracle 是 H1 `SetCurvature` 重存、走 `ProjectCoefficient` 覆盖共享 dof 的另一条路径；L2 `nodes` 读侧改按 MFEM 序累加一次终除，48 顶点逐位；r77 那条 first-wins 断言测试改写为 mean-not-a-copy）。~~D813-3~~（`crates/mesh` 新增 `mfem_geometry_refiner_points(et, times)`（`GlobGeometryRefiner` 七族点阵、MFEM 序；陷阱：棱柱 refiner 域是 `(tri_x,tri_y,layer)` 而 `PrismPk` 挤出优先、金字塔 `Type==0` 分支死代码）+ `Mesh::get_bounding_box(ref)`（无几何臂 = `bounding_box()` 逐位不动）；ex9 改细化后取盒 ⇒ **ex9 数值残差闭合：`ex9-init.gf` 逐字节 = C++（39891 B、diff 0），`ex9-final.gf` max|Δ|=1.0e-08，stdout 仍 0 diff**；修前整解错 3072 值全 |Δ|=1.0）。~~D813-5~~（Tet4 L2 编号：`mfem_l2_slots(Tet4)` = `L2_TetrahedronElement` 循环（`fe_l2.cpp:695`）、`l2_geometry_slots(Tet4)` = `H1TetPk`；`GetNodes()` p=1..3 逐元钉死；`beam-tet` P1/P2 读→写与 MFEM 自身文件**逐字节相同**；`nodes_writer` 的"tet 必须拒"旧断言反转）。~~D808-4 残余~~（`build_tet4_pa_data` 多 QP：`geom_order≥2` ⇒ `tet_rule(2p+1)` + 逐 QP `(J⁻ᵀ,|detJ|,κ)`；曲面 PA vs 装配 **2.437e-2 → 5.137e-16**；直网格 762 行 bit dump **0 差异**）。~~D810-1~~（wg 三模块面路径交 `dg_base::face_point_geom/_3d`，删三份弦长拷贝；**oracle 先行** = MFEM `FaceElementTransformations` 逐面逐 QP 逐侧真值探针：等参 vs MFEM ≤2.5e-15、eip 精确 0；vs 修前弦长曲面 1.053e0；3-D 直网格恰 **2×**（弦长测度本是半值）；散度恒等式 2.78e-1 → 1.2e-15；顺带修 stokes/maxwell 边界稳定项双计）。~~D805-4~~（hex DG 面：`build_face_elem_map` 补 `(8,3)` 臂（修前映射**整张为空**）+ `face_point_geom_3d_quad`（双线性面映射；两陷阱：(1,1) 角是环第三顶点、网格面环有正反绕向需参考级定向修正）+ `face_point_geom_3d_face` 分派器；306 MFEM 面点：**eip/x/W 逐位、nor 2.0e-15**；34/34 面定位一致；tet 路零回归） | **round 78 关闭** |
| **P1（round 78 重登记 / 新债）** | **D814-1 = D805-4b**（hex DG **矩阵级**：`ref_elem_face` 无 QuadFace 族、`assemble_dg_interior_faces` 面型 `dim==2?Line2:Tri3` 的 3-D tet 假设、各 `DgFaceIntegrator` 同面参数化 ⇒ hex DG 面矩阵对拍（round-76 `d805_probe`+`cmp.py` 配方延 3-D hex 夹具）未做——面几何与面→元素查找已通）；**D814-2**（`refine_uniform_3d` **丢**折叠 `L2_T1_3D_P1` 表：`periodic-cube.mesh` 细化两次 ±1/3 vs MFEM ±1，已钉）；**D814-3**（wg 体路径仍走顶点几何 `local_jac`；wg 无 WG 方法真值、无消费方）；**D814-4**（`nd_face_blocks_for_elem` 金字塔行 apex 三角在 base quad 前 vs `HCurlSpace` 先填 base quad 的块序错位——既有、锚局部路径受影响；新通道用绝对槽位不受影响）；**D814-5**（`MakeCartesian3D` 的 `Finalize(true)` 静默旋转 tet 局部顶点序 ⇒ 顶点/体积一致 ≠ 几何一致——曲面 tet MFEM 对比的 trip hazard，oracle 夹具须显式写 connectivity） | 待派 |
| **P1（round 79 关闭账）** | ~~D814-1 = D805-4b~~（**hex DG 矩阵级对拍**：oracle = `tmp/d814/d814_probe.cpp`（gen/dump 分体，规避开本 MFEM 构建 in-memory 网格内部面 `faces_info` 配对丢失的 trip hazard）在 `MakeCartesian3D(2,2,1)`+`SetCurvature(2,false)`+弯折夹具上 dump **15 个矩阵** + `[RULE]` 求积阶审计行（面 dif=2×2/adv=4×4/ela=2×2；o2 dif=3×3/adv=5×5；体 dif=3³/5³、ela=5³，与 bilininteg.cpp 源码公式逐一吻合）。**修复**：①`ref_elem_face` 补 Quad4（QuadL2GL）、`ref_elem_vol` 补 Hex8（HexL2GL）、`interior_faces.rs::local_faces` 补 (8,3) 臂（**同名家族第三例**——D805-4 只修了 `build_face_elem_map` 拷贝，`InteriorFaceList` 在 hex 上一直 panic）；②面型按**面节点数**分派（新 `face_type_of`：2→Line2/3→Tri3/4→Quad4），替换 `dim==2?Line2:Tri3` 5 处 + `assemble_periodic_flux` 新 3-D 臂 + `dg.rs`/`dg_elasticity` 面 per-QP 算术 dim 无关（新 `FaceGeom` 单一真源）；③**`find_face_elem` 潜伏缺陷**："≥2 面节点"启发式在 3-D 把边界项落到错误元素（quad 面第一元素就命中 2 节点）→ 改"全部面节点"；④**`face_point_geom_3d_quad` 潜伏缺陷**（D805-4 引入）：witness-swap 改的是**参数化**而 MFEM（`GetLocalQuadToHexTransformation` + `quad_t::Orient` 点矩阵重排）两邻元恒用规范角点插值、元素相关的只有 `nor` 符号 ⇒ 删角色交换改符号翻转，内面两侧同 ξ 物理点差 **2.5e-1 → ≤1.1e-16**（D805-4 的 34 面金标没抓住：MFEM 写出的边界环全外向、swap 分支死）；⑤`dg.rs` 体项 Qk 规则内部推导（`o+o+dim−1`；hex p=1 起 3³ 点 vs 调用方 2³ 点分叉；2-D 同点数逐位不变）。**实测**：15/15 矩阵 worst rel **1.8e-15**（red：`tmp/d814/prefix_cmp.txt`，标量内面 20–130% 条目错 + 两个 panic）；**ELA 族布局前提更正（㉖）**：round-76 金标是预映射字符串，verbatim dump 对拍需在比较器侧做 byNODES(`comp·n+dof`)→dof-major(`dof·dim+comp`) 置换（ELAV 置换前红 869 条、置换后绿 = 布局而非库）。反回归：d805r78 2/2（面几何金标不动）、d805r76 3/3、d805_d799_3 3/3、fem-assembly **114 靶/1273/0**、ex14 311 行逐字节/ARF 0.956044、ex27 双档 IDENTICAL） | **round 79 关闭** |
| **P1（round 79 新债）** | **D815-1**（`dg.rs` 体项对单纯形 Pk 未走 MFEM `o+o−2` 公式（p=1 = 1 质心点）；现用调用方阶 = 过积分，直网格代数等价、曲面未验；先造单纯形 DG 体项 oracle 再改）、**D815-2**（`dg_hyperbolic` 无 3-D 面路径（2-D tri/quad only）；ex18 3-D 未启用——几何 helper 已就绪）、**D815-3**（`assemble_periodic_flux` 3-D 臂已落地但无 3-D periodic 对拍；需 periodic-cube 级示例对拍）、**D815-4（口径备忘）**（`--workspace --lib` = 18 靶/3004/0/5 与十 crate 口径（10/2654/0/5）并存，基线对比必须同名令；本 MFEM 构建 in-memory 新建网格的内部面 `faces_info` 配对丢失 = MFEM 侧 fixture 生成的 trip hazard，与 D814-5 同类） | 待派 |
| **P1（round 77 重登记 / 新债）** | **D813-1 D807-1 残余**（层仍是锚闭包 **222/246（np2）、222/222/246/234（np4）**，本轮**未切**，理由与配方见 `tmp/d77a/README.md`：① `EntityOwnership` **还走 submesh 线格式**（`mesh_serde.rs:161`/`:466` flag bit 16）⇒ 发布锚单元 `(type, 全局顶点表)` 必须同步 encode/decode，且 `ElementType` **无**数字转换；② 切法是**逐网格全有或全无**（提取侧不知空间阶 ⇒ 不存在"仍拉锚"的空间无关谓词），hex-only 帧会在 tet/prism 面 panic、一刀切会破九空间 `GetTrueVSize` 钉。**残余量化**（1-node 闭包上 facet-anchor miss）：np2 `[9, 70]`、np4 `[42, 51, 89, 72]`，edge-owner miss ≈ 0（owner 侧已被 round-76 通道覆盖）；k≥3 tet 面帧需 `tet_face_slots` 的**无 apex 重心式**重写（数学上无 apex，代码读 `verts[0..3]`））、**D813-2**（**读入器折叠顶点表约定**：`crates/io` 的 L2 `nodes` 读侧取**首个**引用该顶点的单元，MFEM 取**最后**（oracle：`tests/fixtures/d812_mfem_h1p1_periodic-hexagon.mesh.txt` 12/12 行 = last-wins、8/12 = first-wins；periodic-hexagon 在顶点 1,2,3,4 上分叉）。影响 mesh 级顶点表 ⇒ 面变换与包围盒，**不动单元几何**；pin `d812_reader_folded_vertex_table_diverges_from_mfems`）、**D813-3**（**缺曲面几何版 `GetBoundingBox(min,max,ref)`**：fem-rs 只有顶点 min/max；MFEM 的 `Nodes != NULL` 分支按 `GlobGeometryRefiner.Refine(geom, ref)` 采样（`mesh/mesh.cpp:142`）。这是 ex9 `.gf` 数值差的驱动，且依赖 D813-2；consumer = `examples/mfem_ex9_dg_advection.rs` 的 `bb_min`/`bb_max`）、**D813-4**（`read_mfem` 仍拒 `dimension 1` 的 `.mesh`（`data/periodic-segment.mesh`）⇒ 其 `L2_T1_1D_P1` 表永远到不了写者；`Mesh<1>` 容器仅 INLINE 读取器可用）、**D813-5**（order-1 编号已钉 Quad4/Hex8/Tri3/Prism6；**Tet4 仍拒**——仓内无已验证的 MFEM L2 tet 编号） | 待派 |
| **P1（round 75 关闭账）** | ~~D782~~（**二维 serendipity = MFEM `H1Ser_QuadrilateralElement` 逐行端口**：GLL 闭点 / `(p²+3p+6)/2` dof / 真 `S_p` 空间（`xy ∈ S_2`、`x²y² ∉`——旧 span 恰好反了）/ MFEM slot 序与显式公式含 p≥4 Legendre 气泡；**p=1 保持 4-dof 双线性**（MFEM 的 `H1Ser(1)` 退化：5 dof 且第 5 形函数恒零）；真值 1061 行探针 dump，节点表 ≤1e-15 / 形函数 <1e-14 / 导数 <1e-13 **逐条**；**新发现 D809-1：MFEM 自身 p≥4 不满足常数再现**——用 MFEM 自己的 `MassIntegrator` 独立证实 `1ᵀM1 = 1.05667`@p=4（面积 1），忠实继承；**D809-3**：MFEM `H1Ser` 未重载 `Project`（abort））、~~D799-3~~（**三 DG 模块切等参面几何**：`dg_advection`/`dg_elasticity`/`dg_hyperbolic` 原用面角点法向 + 弦长权重 + `phys_to_ref` 反解；改 `face_point_geom` / 新 `FacePointGeom3`；**顺带修 `face_point_geom` 法向定向**——旧实现随面表 a→b 翻转 ⇒ 三角面给**向内**法向（D805-5）；红 5.774e-1(4/16) → **绿 2.802e-14(0/16)**，直边两路线 ≤1e-15 为"直网格恒等"的数值证明）、~~D799-2~~（ex27 4 处硬编码 `Quad4` 改按单元族取——三角网格上会给 L² 路径 4-dof `QuadL2GL` 而空间 3 dof/单元 ⇒ 静默错装配）、~~D800-2~~（**embedded 曲面通量（臂 A）**：根因 = `mesh.dim()` 同时充当 MFEM 的 `spaceDim` 与 `el.GetDim()` ⇒ 3×3 矩阵第三列恒空、`det ≡ 0`；改 `dim×spaceDim` 左逆 `(JᵀJ)⁻¹Jᵀ` ⇒ 切向梯度、测度 `√(EG−F²)`；复用 `assembler::surface_jacobian` **单一真源**；体网格逐字未动；Quad4 曲面 / 1-D 曲线**响亮拒绝**（修前是静默错））、~~D808-1~~（**`quad_q1.wgsl` 是非法 WGSL**——8 处 `if` 表达式（WGSL 无该语法）⇒ 该 GPU 路径**从未编译成功**；主会话用 **naga** 独立证实；改 `select()` + naga 解析 pin；**D777 缺陷类第二次实例**）、~~D783~~（曲面棱柱 PA：`geom_order ≥ 2` 改逐 QP `element_jacobian_at`；**1.612e-1/9.280e-2/1.073e-1 → 8.408e-16/4.060e-16/9.821e-16**）、~~D784~~（多元素共享 dof（共享四边形面 + 中心竖边）p=1..4 逐条目 1.5e-15…3.5e-16，**阴性**）、~~D797-2~~（裁决 **(b) 自洽**：`[-1,1]²` 与 `[0,1]²` 只差 nq=2 Gauss 的精确仿射像，因子在"参考梯度×J⁻ᵀ×detJ×权重"整体抵消；warped J 非对称夹具 8.743e-16 ⇒ 非数值错；帧 pin + 换帧负对照 1.309e0）、~~D781~~（**阴性成文审计**：8 处逐行 + 构造性论证 + 16 次并行装配**逐位 == 串行**、digest 稳定）、~~D808-3~~（**L4 发现/L3 关闭**：`is_periodic_merged` 槽位比较 ⇒ **每个曲面棱柱网格**误判为周期 ⇒ 非协调空间（2 棱柱 p=2 **31 vs 27** dof）；改**集合隶属**；验收摘 ignore 转正）、~~D790-2~~（**验证通过**：九空间 owned split np=2/4 逐位 = MFEM `GetTrueVSize()`；附构造性引理说明为何 owned 不需闭包）、~~D122-2~~（**登记前提被驳**：照字面单层实现 np=4 直接 panic（边 (64,113) 持有者跨 rank 不可答）⇒ fixpoint 是**最小可行层**；静态闭包测试证明 == 全 252 单元；**已 revert** 只留审计注释；重登记 D807-1）、~~D790-3~~（**裁定：登记位置错**——元件侧变换存在且正确（切向对交换 `[[0,1],[1,0]]`、det=−1、`element_signs=+1.0`、`A_ba=RᵀA_abR` ≤1e-10）；缺口在 DP 的标量 `sign_correction` 通道；重登记 D807-2） | **round 75 关闭** |
| **P1（round 74 关闭账）** | ~~D797-1~~（**真缺陷**：HCurlH1WeakDiv 对已物理的梯度又乘 J⁻ᵀ/1/detJ ⇒ 单元矩阵 c⁻³；修后 = MFEM 任意维恒等；**tet + 5 档仿射三角形 MFEM 亲跑逐条目 diff=0**[worst 4.2e-16]；并行路全委托串行、无第二份被积函数）、~~D800-1~~（三条 use_iso 名单统一 l2_error_iso_geometry：直网格逐位、曲面传 geometry_nodes、失配行落 element_jacobian_at；曲面 quad l2err = MFEM）、~~D790-1a~~（use_global_aggregation 死字段 + 6 示例 + 3 测试全删；pex3 双锚逐位）、~~D785~~（joule n_bdr 取全局 mesh0 最大 tag；**有牙探针：4 分区下 rank0/1 本地 max=2<3**；np1/np2 掩码 [1,1,1]）、~~D786~~（HCurl/HDiv 构造接受空本地网格；d803_empty_rank：2 元 × 4 rank 空 rank 全链构造 + 并行包装 12/20 计数一致）、~~D799-1~~（**双根因**：① gen_mesh 末尾 make_periodic 折回[几何快照保 pre-merge 角点 = MFEM 不连续 nodes 语义；is_periodic_merged→build_periodic 周期商；H1 unknowns=302=MFEM]；② **登记未发现**：DG 边界项须走 AddBdrFaceIntegrator 的 Loc1 组合路径而 helpers 是 AddBoundaryIntegrator 风格 ⇒ 两 helper 重写为 face_point_geom 组合。**ex27 九档[dg-rs0..3/h1-rs0..3/dbc25/rhs 隔离] 9/9 IDENTICAL**、ref=0 DG 矩阵 896/896 逐条目=MFEM、ex14 逐字节）| **round 74 关闭** |
