...
| **P1（新，round 70 登记）** | **D749（round 71 头号）**rt/nd print-ref harness **漏上游打印块 `SetFromTrueDofs`**（rt.cpp:386-387 / nd.cpp:391-392）——上游口径 cyl sum rt −2.137667e-4（**vs harness 模型 33×**）/nd 6.932270e-5（5.2×）⇒ **round-69 "红线四值"是 harness 模型值**；端口+harness 补刷新后重钉；**D746**（pex3 L² 行**运行间不确定** ~1e-10，5/5 不同值，14 位锚不可复现——优先根治）；**D743**（serendipity/nurbs hex 臂未迁，被激活前不可信）；**D748**（ex22 3-D `-p1 -o2` 27×[l2_error_hcurl_3d ND1 硬编码]/2-D `-p2 -o2` 22×[RT0 硬编码]/`-p1 -o3` 7×）；**D752**（遗留值列表消元入口族收敛）、**D753**（ex27 -dbc≠0 残余：初值 X=R·x/停机 sqrt/nbc0）、**D754**（ex26 线程不确定 1e-14）、**D755**（D706 覆盖缺口：row-zeroing/复数侧/miniapps）、**D756**（pex34 ranks2 发散 4.5e7）、**D758**（ex34 -hex submesh 规模/ex29 停判据/LOR 门限）、**D759**（mms_verification ND1 帧残留，非 gate）、**D760**（zz_estimator 三维采样点 0.25 = tet 质心复用于 hex）、**D761**（geom_rule Hex20/27 panic + vertex_shapes 空臂）；**D729-D734/D740/D741/D750/D751** 维持 | 待派 |
