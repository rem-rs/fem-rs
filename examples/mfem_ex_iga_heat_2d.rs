//! 2D IGA heat: ∂u/∂t-κΔu=0, implicit Euler. Usage: --nu 12 --nv 12 --p 1 --kappa 1.0 --dt 0.01 --steps 10
...
fn main() {
    ...
    let kappa: f64 = a.iter().position(|x|x=="--kappa").and_then(|i|a.get(i+1)).and_then(|s|s.parse().ok()).unwrap_or(1.0);
    ...
    let dt_stiff = assemble_iga_diffusion_2d(&mesh, kappa * dt, 3);
