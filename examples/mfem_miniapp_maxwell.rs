...
impl MaxwellSolver {
    pub fn new(mesh: Mesh<3>, order: u8, abc_tags: &[i32], dbc_tags: &[i32]) -> Self {
        ...
        let h: f64 = bb.0.iter().zip(bb.1.iter()).map(|(a,b)|b-a).sum::<f64>()/(mesh.n_nodes()as f64).powf(1.0/3.0);
        let dt_val = h / (order as f64 + 1.0) / 3.0e8;
        Self { nd, rt, neg_curl, curl_t, m_eps, m_mu_inv, m_loss,
            e: vec![0.0; n_nd], b: vec![0.0; n_rt], dbc_dofs, dt: dt_val, t: 0.0 }
    }

    pub fn step(&mut self, j_fn: &(dyn Fn(&[f64], f64, &mut [f64]) + Send + Sync)) {
        let dt = self.dt; let cfg = SolverConfig{rtol:1e-12,atol:0.0,max_iter:200,verbose:false,..Default::default()};
        ...
        let mut ce = vec![0.0; self.rt.n_dofs()]; self.neg_curl.spmv(&self.e, &mut ce);
...
fn main() {
    ...
    while ms.t<tf{ms.step(&j_fn);n+=1;if n%sn==0{
        println!("t={:.3e}s E={:.3e}J |E|₂={:.3e} |B|₂={:.3e}",ms.t,ms.energy(),
