...
//! contact integrators (`contact.rs`, `contact_self.rs`).
//!
...
//!    plane at z = -gap_offset
//! 5. Newton solver with contact tangent stiffness from
//!    `assemble_contact_3d_vector()`
//! 6. Verification: penetration check + force equilibrium + contact pressure
//!
//! ## Usage
//! ```text
//! cargo run --example mfem_tribol_contact_patch_test --release
//! cargo run --example mfem_tribol_contact_patch_test -- --n 8 --penalty 1e6 --gap 0.02
//! ```
...
fn body_force_rhs(mesh: &Mesh<3>, total_force: f64) -> Vec<f64> {
    ...
    for i in 0..n {
        rhs[i * 3 + 2] = f; // z-component (downward)
    }
    rhs
}
...
mod tests {
    ...
    fn cube_mesh_tags_match_convention() {
        ...
        assert!(tags.contains(&3), "missing front (y=0)");
        assert!(tags.contains(&4), "missing back (y=1)");
        assert!(tags.contains(&5), "missing left (x=0)");
        assert!(tags.contains(&6), "missing right (x=1)");
    }
    ...
    fn larger_gap_increases_displacement() {
        ...
        assert!(r_large.u_l2 > r_small.u_l2,
            "larger gap should produce larger displacement");
    }
    ...
    fn no_contact_when_gap_is_zero_without_force() {
        ...
        // At u=0 with gap_offset=0, bottom face nodes have z=0 → gap = -0 - 0 = 0 → no penetration
