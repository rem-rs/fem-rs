//! Parallel elastic eigenvalue (pex12). Usage: --n 8 --ranks 2
...
fn main() {
    ...
    ThreadLauncher::new(WorkerConfig::new(r)).launch(move |c| {
        ...
        let mut mass = ParAssembler::assemble_bilinear(&ps, &[&MassIntegrator { rho: 1.0 }], 3);
        ...
        for &d in &bd { let p = dp.permute_dof(d) as usize; if p < dp.n_owned_dofs { mass.apply_dirichlet_par(p, 1.0, &mut tmp); } }
    ...
    println!("pex12: dofs={dof} converged={ok} iters={it}");
}
