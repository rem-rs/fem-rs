//! Example 19 鈥?ALL BUGS FIXED. Schur complement with LU on K_uu + PCG on mass.
...
fn jac_blocks<M:MeshTopology>(m:&M,d:usize,ou:u8,op:u8,qo:u8,mu:f64,u:&[f64],p:&[f64],
    eu:&[Vec<u32>],ep:&[Vec<u32>],nu:usize,np:usize)
    ->(CsrMatrix<f64>,CsrMatrix<f64>,CsrMatrix<f64>){
    let mut cu=CooMatrix::new(nu,nu);let mut cx=CooMatrix::new(nu,np);let mut cy=CooMatrix::new(np,nu);
    for e in 0..m.n_elements()as usize{
        let et=m.element_type(e as u32);let ru_=re(et,ou);let rp_=re(et,op);
        ...
        for a in 0..nfu{let gi=eu[a];for b in 0..nfu{let gj=eu[b];let v=ku[a*nfu+b];cu.add(gi,gj,v);}}
        for a in 0..nfu{let gi=eu[a];for b in 0..np_{let gj=ep[b];let v=kx[a*np_+b];cx.add(gi,gj,v);}}
        for a in 0..np_{let gi=ep[a];for b in 0..nfu{let gj=eu[b];let v=ky[a*nfu+b];cy.add(gi,gj,v);}}
    }
    (cu.into_csr(),cx.into_csr(),cy.into_csr())
}
...
fn run<M:MeshTopology+Clone>(mesh:M,order:u8,mu:f64,rtol:f64,atol:f64,maxit:usize){
    ...
    let cfg=SolverConfig{rtol:1e-8,atol:0.,max_iter:100,verbose:false,print_level:fem_linalg::PrintLevel::Silent};
    let mut u=vec![0.;nu];let mut p=vec![0.;np];
    for s in 0..ns{let xc=dm.dof_coord(s as u32);let fd=idef(xc);let fr=xc.to_vec();for i in 0..d{let idx=s*d+i;if idx<nu{u[idx]=fd[i]-fr[i];}}}
    ...
    if r0<atol{return;}
    for it in 0..maxit{
        if it>0{let rn=nr(&[ru.as_slice(),rp.as_slice()].concat());println!("Newton {:2} ||r||={rn:.5}",it);
            if rn<atol||rn<r0*rtol{println!("converged {it}");return;}}
        let(ku,kup,kpu)=jac_blocks(&mesh,d,order,op,qo,mu,&u,&p,&edu,&edp,nu,np);
        let mut kubc=ku.clone();let mut dm=vec![0.;nu];for i in 0..du.len(){kubc.apply_dirichlet_row_zeroing(du[i].0,0.,&mut dm);}
        // Schur complement: solve J*dx = -r
        // K_uu * du_temp = -r_u  (LU on SPD K_uu)
        let mut du_temp=vec![0.;nu];
        let rhs_u:Vec<f64>=ru.iter().map(|&x|-x).collect();
        match fem_solver::solve_sparse_lu(&kubc,&rhs_u){Ok(x)=>du_temp=x,Err(e)=>{eprintln!("K_uu LU: {e}");break;}}
        // rps = r_p + K_pu * du_temp
        let mut rps=vec![0.;np];for i in 0..np{let mut s=0.;for p in kpu.row_ptr[i]..kpu.row_ptr[i+1]{s+=kpu.values[p]*du_temp[kpu.col_idx[p]as usize];}rps[i]=rp[i]+s;}
        // mass * dp 鈮?S * dp = rps  (PCG on mass, S = K_pu * K_uu^{-1} * K_up is SPD approx)
        let mut dp=vec![0.;np];let _=fem_solver::solve_pcg_gssmoother(&mass,&rps,&mut dp,&cfg);
        // K_uu * du_corr = -K_up * dp
        let mut kupdp=vec![0.;nu];for i in 0..nu{let mut s=0.;for p in kup.row_ptr[i]..kup.row_ptr[i+1]{s+=kup.values[p]*dp[kup.col_idx[p]as usize];}kupdp[i]=-s;}
        let mut du_corr=vec![0.;nu];
        match fem_solver::solve_sparse_lu(&kubc,&kupdp){Ok(x)=>du_corr=x,Err(e)=>{eprintln!("K_uu LU2: {e}");break;}}
        for i in 0..nu{u[i]+=du_temp[i]+du_corr[i];}for i in 0..np{p[i]+=dp[i];}
        res(&mesh,d,order,op,qo,mu,&u,&p,&edu,&edp,&mut ru,&mut rp);
        for&(d,_)in &du{ru[d]=0.;}
    }
    println!("Saving...");
