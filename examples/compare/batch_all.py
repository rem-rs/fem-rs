#!/usr/bin/env python3
"""Full batch comparison: all serial examples (runs in Git Bash)."""
import json, os, re, subprocess, sys

SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
# SCRIPT_DIR = .../fem-pro/fem-rs/examples/compare
FEM_RS = os.path.dirname(os.path.dirname(SCRIPT_DIR))  # .../fem-pro/fem-rs
WIN_DIR = os.path.dirname(FEM_RS)                       # .../fem-pro
DATA_WIN = os.path.join(WIN_DIR, "data")
DATA_CPP = "/home/quan/mfem410/data"
CPP_BIN = "/home/quan/bin/410"
EXE_DIR = os.path.join(FEM_RS, "target", "debug", "examples")

with open(os.path.join(SCRIPT_DIR, "examples.json"), encoding='utf-8') as f:
    CFG = json.load(f)
...
def rust_exe(n): return PMAP.get(n, RMAP.get(n, f"mfem_{n}"))
...
def run_rust(exe, mesh, args, ranks=None):
    ...
    mp = os.path.join(DATA_WIN, mesh).replace("/", "\\")
    cmd = f'"{ep}" -m "{mp}" {args}'
...
def run_cpp(name, mesh, args):
    cpp_bin = "/home/quan/bin/410"
    cpp = f"{cpp_bin}/{name}_cpp"
    mesh_wsl = f"/home/quan/mfem410/data/{mesh}"
    try:
        cmd = ["wsl.exe", "-e", "bash", "-c", f"timeout 60 {cpp} -m {mesh_wsl} {args} 2>&1"]
...
def main():
    ...
    print(f"\nTotal: {len(res)} | OK: {ok} | DIFF: {df} | FAIL: {fl}")
