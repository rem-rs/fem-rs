#!/usr/bin/env python3
"""MFEM 示例 1:1 比对工具 (Git Bash / Windows 原生 Python)"""
import argparse, json, os, re, subprocess, sys

WIN_DIR = r"C:\Users\lilu\works\fem-pro\fem-rs"
DATA_DIR = os.path.join(WIN_DIR, "data")
CPP_DATA = "/home/quan/mfem49/data"
OUT_DIR = os.path.join(WIN_DIR, "tmp", "cmp")
...
def run_rust(exe, mesh, args, ranks=None):
    mp = os.path.join(DATA_DIR, mesh)
    if ranks is not None:
        args = args.replace("{ranks}", str(ranks))
    cmd = [os.path.join(WIN_DIR, "target", "release", "examples", exe + ".exe"), "-m", mp] + args.split()
    try:
...
def run_cpp(name, mesh, args):
    cmd = f"wsl -e bash -c \"timeout 150 ~/bin/{name} -m {CPP_DATA}/{mesh} {args} 2>&1\""
    try:
...
def cmp_val(a, b, tol=1e-6):
    ...
    if isinstance(a, list) and isinstance(b, list):
        ...
        return f"  {'OK' if ok else 'DIFF'} {len(a)} values", ok
...
def compare(name, cfg):
    ...
    if par:
        ...
        dofs = {}
        for np in nps:
            out, rc = run_rust(name, mesh, ra, ranks=np)
    ...
    ro, rc = run_rust(name, mesh, ra)
    co, _ = run_cpp(name + "_cpp", mesh, ca)
    ...
    for m in modes:
        ...
        if m == "eigenvalue":
            ...
            a, b = iters(ro, "Newton"), iters(co, "Newton")
            line, _ = cmp_val(a, b, tol)
            r["lines"].append(f"newton{line}")
        elif m == "objective":
            m1 = re.search(r"(?:objective|J)\s*=\s*([-\d.eE]+)", ro, re.IGNORECASE)
            m2 = re.search(r"(?:objective|J)\s*=\s*([-\d.eE]+)", co, re.IGNORECASE)
            a = float(m1.group(1)) if m1 else None
            b = float(m2.group(1)) if m2 else None
            line, _ = cmp_val(a, b, tol)
            r["lines"].append(f"objective{line}")
        elif m == "marked":
            m1 = re.search(r"Marked\s*(\d+)\s*elements?", ro, re.IGNORECASE)
            m2 = re.search(r"Marked\s*(\d+)\s*elements?", co, re.IGNORECASE)
            a = int(m1.group(1)) if m1 else None
            b = int(m2.group(1)) if m2 else None
            line, _ = cmp_val(a, b, tol)
            r["lines"].append(f"marked{line}")
        elif m == "conv_avg":
            m1 = re.search(r"Average reduction factor\s*=\s*([-\d.eE]+)", ro)
            m2 = re.search(r"Average reduction factor\s*=\s*([-\d.eE]+)", co)
            a = float(m1.group(1)) if m1 else None
            b = float(m2.group(1)) if m2 else None
            line, _ = cmp_val(a, b, tol)
...
def report(results):
    ...
    for r in results:
        status = r["status"]
        icon = {"OK": "OK", "DIFF": "DIFF", "FAIL": "FAIL", "NODATA": "N/A"}.get(status, "?")
        print(f"{r['name']:<25} [{icon}]")
        for l in r["lines"]:
            print(f"  {l}")
    print("="*80)
...
def main():
    ...
    results = [compare(k, cfg[k]) for k in keys if k in cfg]
    report(results)
