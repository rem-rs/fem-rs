...
fn main() {
    ...
    b.add_domain_integrator(DomainSourceIntegrator::new(|_x| 1.0));
    b.assemble();
    let b_vec = b.to_vec();
    ...
    let params = linlvo::SolverParams { rtol: 1e-12, atol: 1e-12, max_iter: 1000, verbose: linlvo::VerboseLevel::Iterations, check_interval: 1 };
    ...
    let err = gf.compute_l2_error(&|_| 0.0, (2 * args.order as u8 + 2).max(3));
    println!("\n|| u_h - u_ex ||_{{L^2}} = {}", err);

    write_mfem_file("refined.mesh", space.mesh()).ok();
    write_mfem_gf_file("sol.gf", dim, &x_full, "H1", args.order as u8, 1, 8).ok();
}
