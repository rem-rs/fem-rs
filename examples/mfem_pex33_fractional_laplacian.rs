//! Example 33p — Fractional Laplacian (parallel)
//!
//! ## Reference: MFEM ex33p
...
fn main() {
    ...
    launcher.launch(move |comm| {
        let base = Mesh::<2>::unit_square_tri(args.n);
        let mesh = if args.refs > 0 { let mut m = base; for _ in 0..args.refs { m = refine_uniform(&m); } m } else { base };
        ...
        let ess = boundary_dofs(&local_mesh, dm, &[1, 2, 3, 4]);
        ...
        if comm.is_root() { println!("pex33: ||u|| = {:.6e}", u.global_norm()); }
    });
}

struct Args { n: usize, refs: usize, np: usize }
fn parse_args() -> Args {
    let mut a = Args { n: 8, refs: 0, np: 2 };
    let mut it = std::env::args().skip(1);
    while let Some(arg) = it.next() {
        match arg.as_str() {
            "--n" => a.n = it.next().and_then(|s| s.parse().ok()).unwrap_or(8),
            "-r" | "--refine" => a.refs = it.next().and_then(|s| s.parse().ok()).unwrap_or(0),
