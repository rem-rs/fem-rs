...
mod tests {
    ...
    #[test]
    fn block_gmres_example_multi_rhs_matches_individual_gmres() {
        let (mat, rhs_1, rhs_2) = build_nonsym_rhs_pair();
        let n = mat.nrows;
        ...
        assert!(res_block.converged, "Block-GMRES example regression did not converge");
        ...
        assert!(res_ref_1.converged, "reference GMRES did not converge for example RHS 1");
        assert!(res_ref_2.converged, "reference GMRES did not converge for example RHS 2");
        ...
        assert!(max_diff_1 < 1e-7, "Block-GMRES example RHS 1 mismatch vs GMRES: {max_diff_1}");
        assert!(max_diff_2 < 1e-7, "Block-GMRES example RHS 2 mismatch vs GMRES: {max_diff_2}");
    }
}
