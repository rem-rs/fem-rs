//! Parallel Robin BC (pex27). Usage: --n 8 --ranks 2 --alpha 10
...
fn main() {
    ...
    let alpha = a.iter().position(|x| x == "--alpha").and_then(|i| a.get(i+1)).and_then(|s| s.parse().ok()).unwrap_or(10.0);
    ...
    ThreadLauncher::new(WorkerConfig::new(r)).launch(move |c| {
        ...
        let mass = MassIntegrator { rho: alpha };
    ...
    println!("pex27(alpha={alpha}): dofs={dof} converged={ok} iters={it}");
}
