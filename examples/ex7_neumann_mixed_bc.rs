...
//! ## Usage
//! ```
//! cargo run --example ex7_neumann_mixed_bc
//! cargo run --example ex7_neumann_mixed_bc -- --n 32
//! cargo run --example ex7_neumann_mixed_bc -- --n 8 --n 16 --n 32  # convergence
//! ```
...
fn solve_one(n: usize, kappa: f64) -> (f64, usize, usize) {
    ...
    let neumann_g = NeumannIntegrator::new(|x: &[f64], _n: &[f64]| {
        // κ * ∂u/∂n at (x,y):
        //   On right (x≈1): κ*(1-2x)*y*(1-y) — the assembler calls this only for tagged faces
        //   On top   (y≈1): κ*(1-2y)*x*(1-x)
