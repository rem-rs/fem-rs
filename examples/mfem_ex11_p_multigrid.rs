...
//! cargo run --example mfem_ex11_p_multigrid -- --n 64 --pmax 4
//! ```
...
use fem_solver::{
    ...
    p_multigrid::{
        PmgHierarchy, PmgPrecond, build_pmg_hierarchy_1d_laplacian,
        solve_vcycle_pmg, fmg_solve,
    },
};
...
fn main() {
    ...
    let n = hierarchy.levels[0].nrows;
    ...
    let res_cg = solve_cg(&hierarchy.levels[0], &rhs, &mut u_cg, &cfg).unwrap_or_else(|e| {
        panic!("CG failed: {}", e)
    });

    // ── PCG + Jacobi ──
    let mut u_pc = vec![0.0; n];
    let res_pc = solve_pcg_jacobi(&hierarchy.levels[0], &rhs, &mut u_pc, &cfg).unwrap_or_else(|e| {
    ...
    let res_pmg = solve_vcycle_pmg(&hierarchy, &rhs, &mut u_pmg, &pmg_precond, &cfg)
        .unwrap_or_else(|e| panic!("p-MG failed: {}", e));

    // ── Full Multigrid (FMG) ──
    let u_fmg = fmg_solve(&hierarchy, &rhs, &pmg_precond, 2)
        .unwrap_or_else(|e| panic!("FMG failed: {}", e));
    ...
    let err_fmg = l2_error_1d(&u_fmg, h);
    ...
    println!("  {:>20} {:>12} {:>14} {:>14.3e}", "FMG", "-", "-", err_fmg);
}
...
fn assemble_rhs_1d(n_elem: usize, p_max: u8) -> Vec<f64> {
    ...
    let n_dofs_per_elem = (p as usize + 1) as usize;
    ...
    for i in 0..n_total {
        rhs[i] = rhs_csr.get(i, 0).unwrap_or(0.0);
    }
...
mod tests {
    ...
    #[test]
    fn ex11_pmg_converges_faster_than_cg() {
        let n = 16;
        let pmax = 3;
        ...
        let mut u_cg = vec![0.0; h.levels[0].nrows];
        let r_cg = solve_cg(&h.levels[0], &rhs, &mut u_cg, &cfg).unwrap();
        let mut u_pmg = vec![0.0; h.levels[0].nrows];
        let prec = PmgPrecond::default();
        let r_pmg = solve_vcycle_pmg(&h, &rhs, &mut u_pmg, &prec, &cfg).unwrap();

        assert!(r_cg.converged);
        assert!(r_pmg.converged);
        assert!(r_pmg.iterations <= r_cg.iterations,
            "p-MG should use fewer iterations: CG={}, pMG={}", r_cg.iterations, r_pmg.iterations);
    }
}
