//! Parallel complex Helmholtz (pex22). Usage: --n 8 --ranks 2 --k 10
...
fn main() {
    ...
    let k = a.iter().position(|x| x == "--k").and_then(|i| a.get(i+1)).and_then(|s| s.parse().ok()).unwrap_or(10.0);
    ...
    ThreadLauncher::new(WorkerConfig::new(r)).launch(move |c| {
        ...
        let mass = MassIntegrator { rho: k * k };
        let mut a = ParAssembler::assemble_bilinear(&ps, &[&diff, &mass], 3);
        let mut rhs = ParAssembler::assemble_linear(&ps, &[&DomainSourceIntegrator::new(|x: &[f64]| {
            let r2 = (x[0]-0.5).powi(2)+(x[1]-0.5).powi(2); (-10.0*r2).exp()
        })], 3);
        ...
        let ok = par_solve_pcg_jacobi(&a, &rhs, &mut u, &SolverConfig { rtol: 1e-6, max_iter: 5000, ..Default::default() }).unwrap();
        *rs.lock().unwrap() = Some((ok.converged, ok.iterations, ps.n_global_dofs()));
    });
    let (ok, it, dof) = res.lock().unwrap().unwrap_or((false, 0, 0));
    println!("pex22(k={k}): dofs={dof} converged={ok} iters={it}");
}
