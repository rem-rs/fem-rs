...
fn main() {
    ...
    // 7. RHS.
    let src_nd = FnVectorSource(Box::new(move |x| { let f = source_3d(x, kappa); [f[0], f[1]] }));
    let rhs_nd = VectorAssembler::assemble_linear(&nd_space, &[&src_nd], quad_order);
    let src_z = FnScalarSource(Box::new(move |x| source_3d(x, kappa)[2]));
    ...
    let mut mat = sys_mat;
    for &d in &nd_bdr { mat.apply_dirichlet_symmetric(d as usize, x[d as usize], &mut rhs); }
    for &d in &h1_bdr { mat.apply_dirichlet_symmetric(n_nd + d as usize, x[n_nd + d as usize], &mut rhs); }
