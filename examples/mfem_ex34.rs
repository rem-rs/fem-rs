...
fn main() {
	...
	let attrs = [1, 2, 3, 4];
	let drive_bdr = [1, 1, 1, 1];

	let solved = StaticMaxwellBuilder::new(space)
		.with_quad_order(4)
		.with_isotropic_coeffs(1.0, 1.0)
		.with_source_fn(source_value)
		.add_tangential_drive_from_marker(&attrs, &drive_bdr, GAMMA, boundary_data)
		.build()
		.solve();

	println!("mfem_ex34 done: converged={}, iters={}, res={:.3e}", solved.solve_result.converged, solved.solve_result.iterations, solved.solve_result.final_residual);
}
