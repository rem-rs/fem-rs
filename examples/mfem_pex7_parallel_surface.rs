//! Parallel Poisson on [0,1]^2 (pex7). Usage: --n 8 --ranks 4
...
fn main() {
    ...
    let n = a.iter().position(|x| x == "--n").and_then(|i| a.get(i+1)).and_then(|s| s.parse().ok()).unwrap_or(6);
    ...
    ThreadLauncher::new(WorkerConfig::new(r)).launch(move |c| {
        ...
        *rs.lock().unwrap() = Some(ok.converged);
    });
    println!("pex7: converged={}", res.lock().unwrap().unwrap_or(false));
}
