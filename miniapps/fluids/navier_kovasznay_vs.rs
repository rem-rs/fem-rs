//! Navier Kovasznay with variable time step — 1:1 serial port of MFEM 4.10
//! `miniapps/fluids/navier/navier_kovasznay_vs.cpp` (+ the shared
...
//!
//! **The time step is adaptive** (`navier_kovasznay_vs.cpp`): each step is
//! first taken *provisionally* (`Step(t, dt, step, provisional = true)`, which
//! leaves the time and the time-step history untouched), the CFL number of the
//! provisional velocity is measured, and
//!
//! * if `error_est = cfl / (cfl_max + cfl_tol) >= 1` the step is **rejected**:
//!   `dt *= 0.5`, `step -= 1` and the step is redone;
//! * otherwise it is **accepted**: `t += dt`, the next step size is predicted
//!   with `eta = (1/(2*error_est))^(1/4)`, `dt *= clamp(eta, 0.1, 1.4)`, and the
//!   new `dt` is queued in the history array via `UpdateTimestepHistory(dt)`.
//!
//! with `cfl_max = 0.8` and `cfl_tol = 1e-4`.  Note that the C++ miniapp
//! queues the *predicted* `dt` (not the accepted one) in the history, so the
//! BDF/EXT coefficients of the next step see `rho1 = dt_next/dt_accepted`.
//!
...
//! * Two C++ quirks are reproduced verbatim: the printed `CFL` is the one
//!   measured from the **provisional** velocity (the miniapp reuses the `cfl`
//!   variable of the acceptance test) while the printed `dt` is the
//!   just-rejected / just-predicted step; and on a rejected *last* step the
//!   loop exits at the `!last_step` condition before the step is redone, so the
//!   printed `err_u`/`err_p` are the previous accepted ones.
//! * The `AddVelDirichletBC(vel, attr)` verbose line (`Adding Velocity
//!   Dirichlet BC to attributes 0 1 2 3 `) is printed, as in the C++
//!   `navier_mms`/`navier_shear` ports — this is one line more than the
//!   sibling `navier_kovasznay` port prints.
//!
//! # Verification
//!
//! Against the serial C++ mirror (`$HOME/work/navier_ser/nkovvs.cpp`, the same
//! 4.10 miniapp source with `ParX → X` and `EnablePA` dropped, built against
//! the WSL MFEM 4.9 library in the full-assembly/`-no-ni` configuration) the
//! step-size controller reproduces the C++ sequence exactly — **`CFL`, `Time`
//! and `dt` agree to all 6 printed digits at every one of the 5 steps**, i.e.
//! the acceptance/rejection test, the predictor and the history bookkeeping are
//! identical:
//!
//! | step | C++ `CFL` / `Time` / `dt` | Rust | C++ `err_u` | Rust `err_u` |
//! |------|---------------------------|------|-------------|--------------|
//! | 1 | 6.23E-02 / 1.00E-03 / 1.40E-03 | identical | 6.24210E-07 | 6.24210E-07 |
//! | 2 | 8.72E-02 / 2.40E-03 / 1.96E-03 | identical | 6.38187E-07 | 6.38186E-07 |
//! | 3 | 1.22E-01 / 4.36E-03 / 2.64E-03 | identical | 6.38733E-07 | 6.38732E-07 |
//! | 4 | 1.64E-01 / 7.00E-03 / 3.29E-03 | identical | 6.38749E-07 | 6.38749E-07 |
//! | 5 | 2.05E-01 / 1.03E-02 / 3.89E-03 | identical | 6.39981E-07 | 6.39982E-07 |
//!
//! `err_p` differs in its last 1–2 digits (e.g. `1.18179E-05` vs `1.18197E-05`)
//! exactly like the verified fixed-step `navier_kovasznay` port — the DOF
//! ordering (and hence the Gauss-Seidel smoother of the pressure solve)
//! differs, which shows up in the pressure at the `1e-4` relative level while
//! the velocity agrees to 5–6 digits.  `MVIN`/`HELM` iteration counts match,
//! `PRES` is within a few iterations.
//!
//! No rejection occurs on the default run (`cfl_max = 0.8` is far above the
//! `~0.2` the Kovasznay flow reaches); the rejection branch is covered by the
//! `predict_dt_factor` unit test.
//!
//! # Sample runs
//!
//! ```text
//! cargo run --release --example navier_kovasznay_vs -- -no-vis
//! cargo run --release --example navier_kovasznay_vs -- -o 4 -rs 2 -no-vis
//! ```
...
/// `cfl_max` / `cfl_tol` of the C++ `main`.
const CFL_MAX: f64 = 0.8;
const CFL_TOL: f64 = 1e-4;
/// `fac_min` / `fac_max` of the C++ `main`.
const FAC_MIN: f64 = 0.1;
const FAC_MAX: f64 = 1.4;

/// The C++ step-size predictor
/// `eta = pow(1/(fac_safety*error_est), 1/(1+3)); dt *= min(fac_max, max(fac_min, eta))`
/// (`fac_safety = 2`), returned as the multiplicative factor.
fn predict_dt_factor(error_est: f64) -> f64 {
    let eta = (1.0 / (2.0 * error_est)).powf(1.0 / 4.0);
    eta.min(FAC_MAX).max(FAC_MIN)
}

// ─── Options (`struct s_NavierContext` in navier_kovasznay_vs.cpp) ───────────
...
fn main() {
    ...
    // `AddVelDirichletBC(vel, attr)`'s verbose line (the C++ prints it before
    // `Setup`; `attr` selects every boundary attribute, so the banner lists the
    // 0-based array indices `0 .. bdr_attributes.Max()`).
    let n_bdr_attr = flowsolver.discretization().bdr_tags.len();
    println!(
    ...
    while !last_step {
        ...
        // Take a provisional step (`provisional = true`: no `t += dt` and no
        // history rotation inside `Step`).
        flowsolver.step(&mut t, dt, step, true);

        // Retrieve the computed provisional velocity and its CFL.
        let u_next = flowsolver.provisional_velocity().to_vec();
        let cfl = flowsolver.compute_cfl(&u_next, dt);

        let error_est = cfl / (CFL_MAX + CFL_TOL);
        if error_est >= 1.0 {
            // Reject the time step.
            println!("Step reached maximum CFL, retrying with smaller step size...");
            dt *= 0.5;
            step -= 1;
        } else {
            // Accept the time step.
            t += dt;
            dt *= predict_dt_factor(error_est);
            // Queue the new time step in the history array.
            flowsolver.update_timestep_history(dt);
        }
        ...
        // The printed `CFL` is the one measured from the *provisional* velocity
        // (the C++ reuses the `cfl` variable of the acceptance test); `dt` is
        // the possibly just-rejected / just-predicted step.
...
mod tests {
    ...
    /// The step-size predictor of `navier_kovasznay_vs.cpp`:
    /// `eta = (1/(2·error_est))^(1/4)`, clamped to `[0.1, 1.4]`.
    #[test]
    fn dt_predictor_matches_the_cpp_formula() {
        // error_est = 1/2 → eta = 1 exactly.
        assert!((predict_dt_factor(0.5) - 1.0).abs() < 1e-15);
        // The Jacobi iteration counts / residuals of `navier_kovasznay` give
        // error_est ≈ 0.0756/0.8001 ≈ 0.09449 → eta ≈ 1.5189 → clamped to 1.4.
        let f = predict_dt_factor(0.0756 / (CFL_MAX + CFL_TOL));
        assert!((f - 1.4).abs() < 1e-15, "f = {f}");
        // A huge CFL ratio (error_est = 4) → eta = (1/8)^(1/4) = 0.5946.
        let f = predict_dt_factor(4.0);
        assert!((f - (0.125_f64).powf(0.25)).abs() < 1e-15, "f = {f}");
        // An absurd ratio is clamped from below.
        assert!((predict_dt_factor(1e8) - FAC_MIN).abs() < 1e-15);
    }

    /// The adaptive driver of this miniapp: a *provisional* step leaves the
    /// time and the time-step history untouched, and `UpdateTimestepHistory`
    /// then queues the *predicted* step — so the next `SetTimeIntegrationCoefficients`
    /// sees `rho1 = dt_next / dt_accepted`, as in the C++ miniapp.
    #[test]
    fn provisional_step_does_not_advance_time_or_history() {
        let d = disc();
        let ic = d
            .vel_space
            .interpolate_vec(&|x| vel_kovasznay(x, 0.0, d.lam).to_vec());
        ...
        let mut t = 0.0;
        s.step(&mut t, 1e-3, 0, true);
        assert_eq!(t, 0.0);
        assert_eq!(s.dthist(), [1e-3, 0.0, 0.0]);
        // The provisional velocity is available and differs from `un` (which
        // still holds the initial condition).
        let cfl = s.compute_cfl(s.provisional_velocity(), 1e-3);
        assert!((cfl - 6.23e-2).abs() < 5e-3, "cfl = {cfl}");

        // Accept the step with a predicted `dt` (as `main` does).
        t += 1e-3;
        let dt_new = 1e-3 * predict_dt_factor(cfl / (CFL_MAX + CFL_TOL));
        s.update_timestep_history(dt_new);
        assert_eq!(t, 1e-3);
        assert_eq!(s.dthist(), [dt_new, 1e-3, 0.0]);
        assert!(dt_new > 1e-3);
        // `un` is now the provisional velocity.
