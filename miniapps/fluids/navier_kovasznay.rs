...
//! * The boundary normal-flux functionals `∫_Γ (v·n) q ds` are assembled in
//!   this file because `fem-assembly` has no `BoundaryNormalLFIntegrator`
//!   equivalent for a *vector* coefficient (its `BoundaryNormalLFIntegrator`
//!   is the scalar `∫ g q ds`) and `ref_elem_face` supports boundary elements
//!   only up to order 4.  The trace of the *volume* basis is used directly,
//!   which is exactly what MFEM's boundary element assembly computes (see
//!   `boundary_normal_lf`).
...
//!   * `mixed::ref_elem_vol` only knows `Quad4` up to order 3, so
//!     `MixedAssembler` cannot build the mixed forms at order 6;
...
impl NavierDiscretization for KovasznayDisc {
    ...
    fn assemble_g_bdr(&self, t: f64) -> Vec<f64> {
        self.boundary_normal_lf(|_e, xp, _phi| vel_kovasznay(xp, t, self.lam))
    }
